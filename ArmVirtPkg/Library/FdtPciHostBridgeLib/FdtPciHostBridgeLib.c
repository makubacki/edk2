  ...
  )
{
  ...
  *Mmio64Size   = 0;
  ...
  if (EFI_ERROR (Status) || Len != 2 * sizeof (UINT64)) {
    DEBUG (
           (EFI_D_ERROR, "%a: 'reg' property not found or invalid\n",
            __FUNCTION__)
           );
  ...
  if (EFI_ERROR (Status) || Len != 2 * sizeof (UINT32)) {
    DEBUG (
           (EFI_D_ERROR, "%a: 'bus-range' not found or invalid\n",
            __FUNCTION__)
           );
    return EFI_PROTOCOL_ERROR;
  }
  ...
  *BusMax = SwapBytes32 (((CONST UINT32 *) Prop)[1]);
  ...
  if (*BusMax < *BusMin || *BusMax - *BusMin == MAX_UINT32 ||
    ...
    DEBUG (
           (EFI_D_ERROR, "%a: invalid 'bus-range' and/or 'reg'\n",
            __FUNCTION__)
           );
    return EFI_PROTOCOL_ERROR;
  }
      ...
      case DTB_PCI_HOST_RANGE_MMIO32:
        ...
        *Mmio32Size = SwapBytes64 (Record->Size);
        ...
        if (Mmio32Translation != 0) {
          DEBUG (
                 (EFI_D_ERROR, "%a: unsupported nonzero MMIO32 translation "
      ...
      case DTB_PCI_HOST_RANGE_MMIO64:
        ...
        *Mmio64Size = SwapBytes64 (Record->Size);
        ...
        if (Mmio64Translation != 0) {
          DEBUG (
                 (EFI_D_ERROR, "%a: unsupported nonzero MMIO64 translation "
  ...
  DEBUG (
         (EFI_D_INFO, "%a: Config[0x%Lx+0x%Lx) Bus[0x%x..0x%x] "
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG (
           (EFI_D_ERROR, "%a: failed to discover PCI host bridge: %r\n",
            __FUNCTION__, Status)
           );
    *Count = 0;
    return NULL;
