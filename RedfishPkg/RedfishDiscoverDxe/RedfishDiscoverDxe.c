  ...
  )
{
  ...
  Status = RestExLibCreateChild (
                                 Instance->Owner,
                                 FixedPcdGetBool (
                            PcdRedfishDiscoverAccessModeInBand
  ...
  )
{
  ...
  ThisNetworkInterface = (EFI_REDFISH_DISCOVER_NETWORK_INTERFACE_INTERNAL *) GetFirstNode (
                                                                                         &mEfiRedfishDiscoverNetworkInterface
                                                                                         );
  while (TRUE) {
    if (CompareMem (
                  (VOID *) &ThisNetworkInterface->MacAddress,
    ...
    ThisNetworkInterface = (EFI_REDFISH_DISCOVER_NETWORK_INTERFACE_INTERNAL *) GetNextNode (
                                                                                         &mEfiRedfishDiscoverNetworkInterface,
  ...
  )
{
  ...
  if (!EFI_ERROR (Status) && Data != NULL && DeviceDescriptor != NULL) {
    ...
    if (DeviceDescriptor->DeviceType == REDFISH_HOST_INTERFACE_DEVICE_TYPE_PCI_PCIE_V2) {
      MacCompareStstus = CompareMem (
                                   &Instance->NetworkInterface->MacAddress,
                                   &DeviceDescriptor->DeviceDescriptor.PciPcieDeviceV2.MacAddress,
                                   6
                                   );
    } else if (DeviceDescriptor->DeviceType == REDFISH_HOST_INTERFACE_DEVICE_TYPE_USB_V2) {
      MacCompareStstus = CompareMem (
                                   &Instance->NetworkInterface->MacAddress,
                                   &DeviceDescriptor->DeviceDescriptor.UsbDeviceV2.MacAddress,
                                   6
                                   );
    } else {
      return EFI_UNSUPPORTED;
    ...
    if (Instance->HostIntfValidation) {
      DEBUG (
            (DEBUG_ERROR, "%a:Send UPnP unicast SSDP to validate this Redfish Host Interface is not supported.\n",
             __FUNCTION__)
            );
  ...
  )
{
  ...
  if (!IsListEmpty (&mRedfishInstanceList)) {
    ...
    do {
      ...
      DiscoveredList = (EFI_REDFISH_DISCOVERED_INTERNAL_LIST *) GetNextNode (
  ...
  if (NewFound || InfoRefresh) {
    if (!InfoRefresh) {
      DiscoveredList =
        (EFI_REDFISH_DISCOVERED_INTERNAL_LIST *) AllocateZeroPool (sizeof (EFI_REDFISH_DISCOVERED_INTERNAL_LIST));
    ...
    if (Uuid != NULL) {
      DiscoveredInstance->Information.Uuid =
        (CHAR16 *) AllocatePool (AsciiStrSize ((const CHAR8 *) Uuid) * sizeof (CHAR16));
      AsciiStrToUnicodeStrS (
                            (const CHAR8 *) Uuid,
                            DiscoveredInstance->Information.Uuid,
    ...
    if (Os != NULL) {
      DiscoveredInstance->Information.Os =
        (CHAR16 *) AllocatePool (AsciiStrSize ((const CHAR8 *) Os) * sizeof (CHAR16));
      AsciiStrToUnicodeStrS (
                            (const CHAR8 *) Os,
                            DiscoveredInstance->Information.Os,
      ...
      DEBUG (
            (DEBUG_INFO, "Redfish service OS: %s, Version:%s.\n", DiscoveredInstance->Information.Os,
    ...
    if (Product != NULL && ProductVer != NULL) {
      ...
      DEBUG (
            (DEBUG_INFO, "Redfish service product: %s, Version:%s.\n", DiscoveredInstance->Information.Product,
             DiscoveredInstance->Information.ProductVer)
            );
    }
    ...
    if (RedfishLocation == NULL) {
      ...
      IP4_COPY_ADDRESS (
                      (VOID *) &DiscoveredInstance->Information.RedfishHostIpAddress.v4,
                      (VOID *) &Instance->TargetIpAddress.v4
                      );
    }
  ...
  if (gRequiredProtocol[ProtocolType].GetSubnetInfo != NULL && Instance->GotSubnetInfo == FALSE) {
    ...
    if (EFI_ERROR (Status)) {
      ...
      if (CheckIsIpVersion6 (Instance)) {
        ...
        DEBUG (
               (DEBUG_INFO, "   IPv6 Subnet ID:%d, Prefix length: %d.\n",
        ...
        DEBUG (
               (DEBUG_INFO, "   IPv4 Subnet:%d.%d.%d.%d Subnet mask: %d.%d.%d.%d.\n",
  ...
  *NetworkIntfInstances = NULL;
  ...
  ThisNetworkInterface =
    (EFI_REDFISH_DISCOVER_NETWORK_INTERFACE *) AllocateZeroPool (
                                                                                      sizeof (
                                                                                            EFI_REDFISH_DISCOVER_NETWORK_INTERFACE) * mNumNetworkInterface
                                                                                      );
  ...
  *NetworkIntfInstances    = ThisNetworkInterface;
  ...
  while (TRUE) {
    ...
    if (!ThisNetworkInterface->IsIpv6) {
      IP4_COPY_ADDRESS (&ThisNetworkInterface->SubnetId.v4, &ThisNetworkInterfaceIntn->SubnetAddr.v4); // IPv4 subnet
                                                                                                       // information.
    } else {
      IP6_COPY_ADDRESS (&ThisNetworkInterface->SubnetId.v6, &ThisNetworkInterfaceIntn->SubnetAddr.v6); // IPv6 subnet
                                                                                                       // information in
                                                                                                       // IPv6 address
                                                                                                       // information.
    }
    ...
    ThisNetworkInterfaceIntn = (EFI_REDFISH_DISCOVER_NETWORK_INTERFACE_INTERNAL *) GetNextNode (
                                                                                             &mEfiRedfishDiscoverNetworkInterface,
  ...
  )
{
  ...
  if (TargetNetworkInterface != NULL) {
    ...
    TargetNetworkInterfaceInternal = (EFI_REDFISH_DISCOVER_NETWORK_INTERFACE_INTERNAL *) GetFirstNode (
                                                                                                     &mEfiRedfishDiscoverNetworkInterface
                                                                                                     );
  ...
  for (NetworkInterfacesIndex = 0; NetworkInterfacesIndex < NumNetworkInterfaces; NetworkInterfacesIndex++) {
    ...
    Status2     = EFI_SUCCESS;
    NewInstance = FALSE;
    Instance    = GetInstanceByOwner (
                                      ImageHandle,
                                      TargetNetworkInterfaceInternal,
                                      Flags & ~EFI_REDFISH_DISCOVER_VALIDATION
                                      );                                                                                   //
                                                                                                                           // Check
                                                                                                                           // if
                                                                                                                           // we
    ...
    if (Instance == NULL) {
      ...
      Instance =
        (EFI_REDFISH_DISCOVERED_INTERNAL_INSTANCE *) AllocateZeroPool (sizeof (EFI_REDFISH_DISCOVERED_INTERNAL_INSTANCE));
      if (Instance == NULL) {
    ...
    if (TargetNetworkInterfaceInternal->StrMacAddr != NULL) {
      DEBUG (
           (DEBUG_INFO, "%a:Acquire Redfish service on network interface MAC address:%s.\n", __FUNCTION__,
    ...
    if ((Flags & EFI_REDFISH_DISCOVER_SSDP) != 0) {
      ...
      if (EFI_ERROR (Status1) && EFI_ERROR (Status2)) {
        ...
        DEBUG (
              (DEBUG_ERROR, "%a:Something wrong on Redfish service discovery Status1=%x, Status2=%x.\n", __FUNCTION__,
               Status1, Status2)
              );
      } else {
        if (NewInstance) {
    ...
    if (TargetNetworkInterface == NULL) {
      ...
      //
      TargetNetworkInterfaceInternal = (EFI_REDFISH_DISCOVER_NETWORK_INTERFACE_INTERNAL *) GetNextNode (
                                                                                                     &mEfiRedfishDiscoverNetworkInterface,
  ...
  )
{
  ...
  for (NumService = 0; NumService < InstanceList->NumberOfServiceFound; NumService++) {
    ...
    do {
      ...
      DiscoveredRedfishInstance = (EFI_REDFISH_DISCOVERED_INTERNAL_LIST *) GetNextNode (
                                                                                     &mRedfishInstanceList,
...
ReleaseNext:;
    ...
    //
    ThisRedfishInstance =
      (EFI_REDFISH_DISCOVERED_INSTANCE *) ((UINT8 *) ThisRedfishInstance + sizeof (EFI_REDFISH_DISCOVERED_INSTANCE));
  }
  ...
  *IsNewInstance = TRUE;
  if (!IsListEmpty ((const LIST_ENTRY *) &mEfiRedfishDiscoverNetworkInterface)) {
    ...
    if (ThisNetworkInterface != NULL) {
      while (TRUE) {
        if ((CompareMem (
                        (CONST VOID *) &ThisNetworkInterface->MacAddress.Addr,
                        (CONST VOID *) &MacAddress.Addr,
                        HwAddressSize
                        ) == 0) &&
  ...
  )
{
  ...
  for (Index = 0; Index < (sizeof (gRequiredProtocol) / sizeof (REDFISH_DISCOVER_REQUIRED_PROTOCOL)); Index++) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      if (EFI_ERROR (Status)) {
        DEBUG (
             (DEBUG_ERROR, "%a: %s is found on this controller handle.\n", __FUNCTION__,
              gRequiredProtocol[Index].ProtocolName)
             );
        return EFI_SUCCESS;
      }
    }
  ...
  )
{
  ...
  gBS->RestoreTPL (OldTpl);
      ...
      // Redfish Service Discover protocol will match REST EX to the corresponding
      // EFI_REDFISH_DISCOVER_NETWORK_INTERFACE_INTERNAL
      // when discovery.
  ...
  for (Index = 0; Index < (sizeof (gRequiredProtocol) / sizeof (REDFISH_DISCOVER_REQUIRED_PROTOCOL)); Index++) {
    ...
    if (!EFI_ERROR (Status)) {
      if (gRequiredProtocol[Index].ProtocolType != ProtocolTypeRestEx) {
        ...
        OldTpl = gBS->RaiseTPL (EFI_REDFISH_DISCOVER_NETWORK_INTERFACE_TPL);
        RestExInstance = (EFI_REDFISH_DISCOVER_REST_EX_INSTANCE_INTERNAL *) GetFirstNode (
        ...
        while (TRUE) {
          ...
          RestExInstance = (EFI_REDFISH_DISCOVER_REST_EX_INSTANCE_INTERNAL *) GetNextNode (
                                                                                        &mEfiRedfishDiscoverRestExInstance,
