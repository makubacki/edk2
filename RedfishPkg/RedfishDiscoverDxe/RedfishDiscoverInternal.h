...
typedef struct {
  ...
  EFI_HANDLE                                     OpenDriverControllerHandle; ///< The controller handle to open network
                                                                             ///< protocol.
  UINTN                                          HwAddressSize;              ///< The size of network interface hardware
                                                                             ///< address.
  EFI_MAC_ADDRESS                                MacAddress;                 ///< MAC address of network interface.
  CHAR16                                         *StrMacAddr;                ///< String to MAC address of network
                                                                             ///< interface.
  BOOLEAN                                        GotSubnetInfo;              ///< Indicates sub net information is
                                                                             ///< retrieved.
  ...
  UINT32                                         NetworkProtocolType;          ///< Network protocol type. Refer to
                                                                               ///< NETWORK_INTERFACE_PROTOCOL_TYPE.
  REDFISH_DISCOVER_NETWORK_INTERFACE_PROTOCOL    NetworkInterfaceProtocolInfo; ///< Network interface protocol
                                                                               ///< information.
  EFI_HANDLE                                     RestExHandle;                 ///< REST EX handle associated with this
                                                                               ///< network interface.
...
typedef struct {
  ...
  EFI_GUID                                *DiscoveredProtocolGuid;             ///< Protocol interface GUID use to
                                                                               ///< install identifier.
  EFI_REDFISH_DISCOVER_GET_SUBNET_INFO    GetSubnetInfo;                       ///< Function of getting subnet
                                                                               ///< information.
...
typedef struct {
  LIST_ENTRY                                         Entry;             ///< Link list entry.
  EFI_HANDLE                                         Owner;             ///< The owner owns this Redfish service
                                                                        ///< discovery.
  ...
  EFI_REDFISH_DISCOVERED_TOKEN                       *DiscoverToken;    ///< Token used to signal when Redfish service
                                                                        ///< is discovered.
  ...
  //
  BOOLEAN                                            HostIntfValidation; ///< Indicates whether to validate Redfish Host
                                                                         ///< interface.
  EFI_IP_ADDRESS                                     TargetIpAddress;    ///< Target IP address reported in Redfish Host
                                                                         ///< interface.
