...
getOpResult (
  redfishPayload *payload, const char *propName, const char *op, const char *value, EFI_HTTP_STATUS_CODE **StatusCode
  );
...
static redfishPayload *
createCollection (
  redfishService *service, size_t count, redfishPayload **payloads
  );

static json_t *
json_object_get_by_index (
  json_t *json, size_t index
  );
...
char *
payloadToString (
  redfishPayload *payload, bool prettyPrint
  )
{
  size_t  flags = 0;
...
createRedfishPayload (
  json_t *value, redfishService *service
  )
{
  redfishPayload  *payload;
...
getPayloadByNodeName (
  redfishPayload *payload, const char *nodeName, EFI_HTTP_STATUS_CODE **StatusCode
  )
{
  json_t      *value;
  json_t      *odataId;
...
redfishPayload *
getPayloadByIndex (
  redfishPayload *payload, size_t index, EFI_HTTP_STATUS_CODE **StatusCode
  )
{
  json_t      *value = NULL;
  json_t      *odataId;
  ...
  *StatusCode = NULL;
  ...
  redfishPayload  *members = getPayloadByNodeName (payload, "Members", StatusCode);
    if ((*StatusCode == NULL && members == NULL) ||
        (*StatusCode != NULL &&
         (**StatusCode < HTTP_STATUS_200_OK || **StatusCode > HTTP_STATUS_206_PARTIAL_CONTENT))) {
      return members;
    }
...
redfishPayload *
getPayloadForPath (
  redfishPayload *payload, redPathNode *redpath, EFI_HTTP_STATUS_CODE **StatusCode
  )
{
...
getPayloadForPathString (
  redfishPayload *payload, const char *string, EFI_HTTP_STATUS_CODE **StatusCode
  )
{
  redPathNode     *redpath;
  redfishPayload  *ret;
...
redfishPayload *
patchPayload (
  redfishPayload *target, redfishPayload *payload, EFI_HTTP_STATUS_CODE **StatusCode
  )
{
  json_t  *json;
  char    *content;
...
postContentToPayload (
  redfishPayload *target, const char *data, size_t dataSize, const char *contentType, EFI_HTTP_STATUS_CODE **StatusCode
  )
{
  json_t  *json;
  char    *uri;
