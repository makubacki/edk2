...
static int
initRest (
  redfishService *service, void *restProtocol
  );

static redfishService *
createServiceEnumeratorNoAuth (
  const char *host, const char *rootUri, bool enumerate, unsigned int flags, void *restProtocol
  );
...
static char *
makeUrlForService (
  redfishService *service, const char *uri
  );

static json_t *
getVersions (
  redfishService *service, const char *rootUri
  );
...
json_t *
getUriFromService (
  redfishService *service, const char *uri, EFI_HTTP_STATUS_CODE **StatusCode
  )
{
  char                   *url;
  json_t                 *ret;
...
json_t *
patchUriFromService (
  redfishService *service, const char *uri, const char *content, EFI_HTTP_STATUS_CODE **StatusCode
  )
{
  char                   *url;
  json_t                 *ret;
...
getRedfishServiceRoot (
  redfishService *service, const char *version, EFI_HTTP_STATUS_CODE **StatusCode
  )
{
  json_t      *value;
  json_t      *versionNode;
...
redfishPayload *
getPayloadByPath (
  redfishService *service, const char *path, EFI_HTTP_STATUS_CODE **StatusCode
  )
{
  redPathNode     *redpath;
  redfishPayload  *root;
  ...
  )
{
  ...
  if(sessionPayload == NULL || StatusCode == NULL || *StatusCode < HTTP_STATUS_200_OK ||
  ...
  )
{
  ...
  if (ret == NULL || StatusCode == NULL || *StatusCode < HTTP_STATUS_200_OK ||
      *StatusCode > HTTP_STATUS_206_PARTIAL_CONTENT) {
    if (ret != NULL) {
      json_decref (ret);
    }
