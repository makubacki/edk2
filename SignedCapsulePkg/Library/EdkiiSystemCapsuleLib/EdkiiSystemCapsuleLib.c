  ...
  )
{
  ...
  while ((UINTN) FvHeader < (UINTN) FdStart + FdSize - 1) {
    ...
    if (FvHeader->FvLength > FvSize) {
      DEBUG (
           (DEBUG_ERROR, "GetFfsByName - FvSize: 0x%08x, MaxSize - 0x%08x\n", (UINTN) FvHeader->FvLength,
            (UINTN) FvSize)
           );
      return FALSE;
    }
  ...
  )
{
  ...
  *LastAttemptStatus = LAST_ATTEMPT_STATUS_ERROR_INVALID_FORMAT;
  ...
  if (ImageAuth->AuthInfo.Hdr.wRevision != 0x0200) {
    DEBUG (
         (DEBUG_ERROR, "ExtractAuthenticatedImage - wRevision: 0x%02x, expect - 0x%02x\n",
  ...
  if (ImageAuth->AuthInfo.Hdr.wCertificateType != WIN_CERT_TYPE_EFI_GUID) {
    DEBUG (
         (DEBUG_ERROR, "ExtractAuthenticatedImage - wCertificateType: 0x%02x, expect - 0x%02x\n",
  ...
  )
{
  ...
  *LastAttemptVersion = ImageFmpInfo->Version;
  ...
  DEBUG (
       (DEBUG_INFO, "Current LowestSupportedImageVersion - 0x%x\n",
