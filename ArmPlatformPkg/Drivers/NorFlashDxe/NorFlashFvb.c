  ...
  )
{
  ...
  // FirmwareVolumeHeader->FvLength is declared to have the Variable area AND the FTW working area AND the FTW Spare
  // contiguous.
  ...
  )
{
  ...
  if (  (FwVolHeader->Revision  != EFI_FVH_REVISION)
    ...
    DEBUG (
           (EFI_D_INFO, "%a: No Firmware Volume header present\n",
            __FUNCTION__)
           );
  ...
  if( CompareGuid (&FwVolHeader->FileSystemGuid, &gEfiSystemNvDataFvGuid) == FALSE ) {
    DEBUG (
           (EFI_D_INFO, "%a: Firmware Volume Guid non-compatible\n",
            __FUNCTION__)
           );
    return EFI_NOT_FOUND;
  }
  ...
  if (Checksum != 0) {
    DEBUG (
           (EFI_D_INFO, "%a: FV checksum is invalid (Checksum:0x%X)\n",
  ...
  if (!CompareGuid (&VariableStoreHeader->Signature, &gEfiVariableGuid) &&
    ...
    DEBUG (
           (EFI_D_INFO, "%a: Variable Store Guid non-compatible\n",
            __FUNCTION__)
           );
    return EFI_NOT_FOUND;
  }
  ...
  if (VariableStoreHeader->Size != VariableStoreLength) {
    DEBUG (
           (EFI_D_INFO, "%a: Variable Store Length does not match\n",
            __FUNCTION__)
           );
    return EFI_NOT_FOUND;
  }
  ...
  )
{
  ...
  FlashFvbAttributes = (EFI_FVB_ATTRIBUTES_2) (
                                               ...
                                               EFI_FVB2_STICKY_WRITE     | // A block erase is required to flip bits
                                                                           // into EFI_FVB2_ERASE_POLARITY
                                               EFI_FVB2_MEMORY_MAPPED    | // It is memory mapped
  ...
  )
{
  ...
  DEBUG (
        (DEBUG_BLKIO, "FvbGetBlockSize(Lba=%ld, BlockSize=0x%x, LastBlock=%ld)\n", Lba, Instance->Media.BlockSize,
         Instance->Media.LastBlock)
        );

  if (Lba > Instance->Media.LastBlock) {
    DEBUG (
          (EFI_D_ERROR, "FvbGetBlockSize: ERROR - Parameter LBA %ld is beyond the last Lba (%ld).\n", Lba,
           Instance->Media.LastBlock)
          );
  ...
  )
{
  ...
  DEBUG (
        (DEBUG_BLKIO, "FvbRead(Parameters: Lba=%ld, Offset=0x%x, *NumBytes=0x%x, Buffer @ 0x%08x)\n",
  ...
  DEBUG (
        (DEBUG_BLKIO, "FvbRead: Check if (Offset=0x%x + NumBytes=0x%x) <= BlockSize=0x%x\n", Offset, *NumBytes,
         BlockSize)
        );
  ...
  if ((Offset               >= BlockSize) ||
    ...
    DEBUG (
          (EFI_D_ERROR, "FvbRead: ERROR - EFI_BAD_BUFFER_SIZE: (Offset=0x%x + NumBytes=0x%x) > BlockSize=0x%x\n",
           Offset, *NumBytes, BlockSize)
          );
  ...
  )
{
  ...
  do {
    ...
    while (NumOfLba > 0) {
      ...
      // Erase it
      DEBUG (
            (DEBUG_BLKIO, "FvbEraseBlocks: Erasing Lba=%ld @ 0x%08x.\n", Instance->StartLba + StartingLba,
             BlockAddress)
            );
