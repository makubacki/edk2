  ...
  )
{
  ...
  if (ImageContext->PdbPointer) {
        ...
        DEBUG (
          (EFI_D_LOAD | EFI_D_INFO, "load /a /ni /np %a &0x%p\n",
        ...
        DEBUG (
          (EFI_D_LOAD | EFI_D_INFO, "add-symbol-file %a 0x%p\n",
           DeCygwinPathIfNeeded (ImageContext->PdbPointer, Temp, sizeof (Temp)),
           (UINTN) (ImageContext->ImageAddress + ImageContext->SizeOfHeaders))
          );
 #endif
 ...
 #else
      DEBUG (
          (EFI_D_LOAD | EFI_D_INFO, "Loading driver at 0x%11p EntryPoint=0x%11p\n",
  ...
  )
{
  ...
  if (ImageContext->PdbPointer) {
      ...
      // Print out the command for the RVD debugger to load symbols for this image
      DEBUG (
          (DEBUG_LOAD | DEBUG_INFO, "unload symbols_only %a\n",
           DeCygwinPathIfNeeded (ImageContext->PdbPointer, Temp, sizeof (Temp)))
          );
 #elif __GNUC__
      // This may not work correctly if you generate PE/COFF directly as then the Offset would not be required
      DEBUG (
          (DEBUG_LOAD | DEBUG_INFO, "remove-symbol-file %a 0x%08x\n",
           DeCygwinPathIfNeeded (ImageContext->PdbPointer, Temp, sizeof (Temp)),
           (UINTN) (ImageContext->ImageAddress + ImageContext->SizeOfHeaders))
          );
 #else
