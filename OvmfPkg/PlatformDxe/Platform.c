  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG (
           (DEBUG_ERROR, "%a: BlockToConfig(): %r, Progress=\"%s\"\n",
  ...
  )
{
  ...
  DEBUG (
         (DEBUG_VERBOSE, "%a: Configuration=\"%s\"\n", __FUNCTION__,
          Configuration)
         );
  ...
  if (EFI_ERROR (Status)) {
    DEBUG (
           (DEBUG_ERROR, "%a: ConfigToBlock(): %r, Progress=\"%s\"\n",
            __FUNCTION__, Status,
...
{
  DEBUG (
         (DEBUG_VERBOSE, "%a: Action=0x%Lx QuestionId=%d Type=%d\n",
