...
BBS_TYPE                          mBbsType[] = {
  BBS_FLOPPY, BBS_HARDDISK, BBS_CDROM, BBS_EMBED_NETWORK, BBS_BEV_DEVICE, BBS_UNKNOWN
};
  ...
  )
{
  ...
  HiiGetBrowserData (&mLegacyBootOptionGuid, mLegacyBootStorageName, sizeof (LEGACY_BOOT_NV_DATA),
    (UINT8 *)CurrentNVMap);
  DisMap = mLegacyBootOptionPrivate->MaintainMapData->DisableMap;
  ...
  //
  HiiSetBrowserData (&mLegacyBootOptionGuid,
  ...
  )
{
  ...
  DisMap =
    ZeroMem (mLegacyBootOptionPrivate->MaintainMapData->DisableMap,
  ...
  )
{
  ...
  for (Index = 0; Index < BootOptionCount; Index++) {
    ...
    NewLegacyDevContext->Description = AllocateCopyPool (StrSize (BootOption[Index].Description),
