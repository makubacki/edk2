...
typedef struct {
  ...
  UINT8     NumberBbsEntries;            ///< The number of valid BBS table entries upon entry and exit. The IBV code
                                         ///< may
                                         ///< increase this number, if BBS-compliant devices also hook INTs in order to
                                         ///< force the
                                         ///< OpROM BIOS Setup to be executed.
  UINT32    BbsTablePointer;             ///< A pointer to the BBS table.
  UINT16    RuntimeSegment;              ///< The segment where the OpROM can be relocated to. If this value is 0x0000,
                                         ///< this
                                         ///< means that the relocation of this run time code is not supported.
                                         ///< Inconsistent with specification here:
                                         ///< The member's name "OpromDestinationSegment" [defined in Intel Framework
                                         ///< Compatibility Support Module Specification / 0.97 version]
...
///
typedef struct {
  DEVICE_PRODUCER_SERIAL      Serial[4];            ///< Data for serial port x. Type DEVICE_PRODUCER_SERIAL is defined
                                                    ///< below.
  DEVICE_PRODUCER_PARALLEL    Parallel[3];          ///< Data for parallel port x. Type DEVICE_PRODUCER_PARALLEL is
                                                    ///< defined below.
...
typedef struct {
  ...
  UINT32                         SmbiosTable;                     ///< The location of the SMBIOS table in EFI memory. <
                                                                  ///< 4G range.
  ...
  UINT32                         NumberE820Entries;               ///< Number of E820 entries. The number can change
                                                                  ///< from the
                                                                  ///< Compatibility16InitializeYourself() function.
  //
  // Controller & Drive Identify[2] per controller information
  //
  HDD_INFO                       HddInfo[MAX_IDE_CONTROLLER];     ///< Hard disk drive information, including raw
                                                                  ///< Identify Drive data.
  UINT32                         NumberBbsEntries;                ///< Number of entries in the BBS table
  UINT32                         BbsTable;                        ///< A pointer to the BBS table. Type BBS_TABLE is
                                                                  ///< defined below.
  UINT32                         SmmTable;                        ///< A pointer to the SMM table. Type SMM_TABLE is
                                                                  ///< defined below.
  UINT32                         OsMemoryAbove1Mb;                ///< The amount of usable memory above 1 MB, i.e. E820
                                                                  ///< type 1 memory. This value can
                                                                  ///< differ from the value in
                                                                  ///< EFI_TO_COMPATIBILITY16_INIT_TABLE as more
                                                                  ///< memory may have been discovered.
  UINT32                         UnconventionalDeviceTable;       ///< Information to boot off an unconventional device
                                                                  ///< like a PARTIES partition. Type
