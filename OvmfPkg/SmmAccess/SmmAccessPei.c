  ...
  )
{
  ...
  if (HostBridgeDevId != INTEL_Q35_MCH_DEVICE_ID) {
    DEBUG (
           (DEBUG_ERROR, "%a: no SMRAM with host bridge DID=0x%04x; only "
  ...
  if ((EsmramcVal & RegMask8) != RegMask8) {
    DEBUG (
           (DEBUG_ERROR, "%a: this Q35 implementation lacks SMRAM\n",
            __FUNCTION__)
           );
    goto WrongConfig;
  }
  ...
  UINTN  Idx;
    ...
    DEBUG (
           (DEBUG_VERBOSE, "%a: SMRAM map follows, %d entries\n", __FUNCTION__,
            (INT32) Count)
           );
    DEBUG (
           (DEBUG_VERBOSE, "% 20a % 20a % 20a % 20a\n", "PhysicalStart(0x)",
            "PhysicalSize(0x)", "CpuStart(0x)", "RegionState(0x)")
           );
    for (Idx = 0; Idx < Count; ++Idx) {
      DEBUG (
             (DEBUG_VERBOSE, "% 20Lx % 20Lx % 20Lx % 20Lx\n",
