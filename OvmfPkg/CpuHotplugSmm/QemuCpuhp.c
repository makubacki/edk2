  ...
  *ToUnplugCount = 0;
  ...
  APIC_ID  NewApicId;
    ...
    if (PendingSelector < CurrentSelector) {
      DEBUG (
             (DEBUG_VERBOSE, "%a: CurrentSelector=%u PendingSelector=%u: "
    ...
    if ((CpuStatus & QEMU_CPUHP_STAT_INSERT) != 0) {
      ...
      if ((CpuStatus & QEMU_CPUHP_STAT_ENABLED) == 0 ||
        ...
        DEBUG (
               (DEBUG_ERROR, "%a: CurrentSelector=%u CpuStatus=0x%x: "
      ...
      DEBUG (
             (DEBUG_VERBOSE, "%a: CurrentSelector=%u: insert\n", __FUNCTION__,
              CurrentSelector)
             );
      ...
      DEBUG (
             (DEBUG_VERBOSE, "%a: CurrentSelector=%u: fw_remove\n",
              __FUNCTION__, CurrentSelector)
             );
      ...
      //
      DEBUG (
             (DEBUG_VERBOSE, "%a: CurrentSelector=%u: remove (ignored)\n",
              __FUNCTION__, CurrentSelector)
             );
      ...
      DEBUG (
             (DEBUG_VERBOSE, "%a: CurrentSelector=%u: no event\n",
              __FUNCTION__, CurrentSelector)
             );
      break;
    }
    ...
    if (ExtendIds != NULL) {
      ...
      DEBUG (
             (DEBUG_VERBOSE, "%a: ApicId=" FMT_APIC_ID "\n", __FUNCTION__,
              NewApicId)
             );
      if (ExtendSels != NULL) {
  ...
  DEBUG (
         (DEBUG_VERBOSE, "%a: PluggedCount=%u ToUnplugCount=%u\n",
