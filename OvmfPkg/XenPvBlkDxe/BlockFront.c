  ...
  )
{
  ...
  if (Status != XENSTORE_STATUS_SUCCESS || Value > MAX_UINT16) {
    DEBUG (
           (DEBUG_ERROR, "XenPvBlk: Failed to get backend-id (%d)\n",
            Status)
           );
    goto Error;
  }
...
Again:
  ...
  if ((UINT32) Value % 512 != 0) {
    ...
    //
    DEBUG (
           (DEBUG_ERROR, "XenPvBlk: Unsupported sector-size value %Lu, "
  ...
  DEBUG (
         (DEBUG_INFO, "XenPvBlk: New disk with %ld sectors of %d bytes\n",
