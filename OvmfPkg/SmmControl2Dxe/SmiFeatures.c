  ...
  )
{
  ...
  if (RETURN_ERROR (
    ...
    DEBUG (
           (DEBUG_INFO, "%a: SMI feature negotiation unavailable\n",
            __FUNCTION__)
           );
    return FALSE;
  }
  ...
  if (SupportedFeaturesSize != sizeof mSmiFeatures ||
    ...
    DEBUG (
           (DEBUG_ERROR, "%a: size mismatch in feature negotiation\n",
            __FUNCTION__)
           );
    goto FatalError;
  }
  ...
  if (QemuFwCfgRead8 () != 1) {
    DEBUG (
           (DEBUG_ERROR, "%a: negotiation failed for feature bitmap 0x%Lx\n",
  ...
  if ((mSmiFeatures & ICH9_LPC_SMI_F_BROADCAST) == 0) {
    ...
    if (RETURN_ERROR (PcdSet64S (PcdCpuSmmApSyncTimeout, 1000000)) ||
      ...
      DEBUG (
             (DEBUG_ERROR, "%a: PiSmmCpuDxeSmm PCD configuration failed\n",
              __FUNCTION__)
             );
      goto FatalError;
    }
  ...
  if ((mSmiFeatures & ICH9_LPC_SMI_F_CPU_HOTPLUG) == 0) {
    ...
    DEBUG (
           (DEBUG_INFO, "%a: CPU hotplug with SMI negotiated\n",
            __FUNCTION__)
           );
  }
  ...
  if ((mSmiFeatures & ICH9_LPC_SMI_F_CPU_HOT_UNPLUG) == 0) {
    ...
    DEBUG (
           (DEBUG_INFO, "%a: CPU hot-unplug with SMI negotiated\n",
            __FUNCTION__)
           );
  }
  ...
  )
{
  ...
  DEBUG (
         (DEBUG_VERBOSE, "%a: SMI feature negotiation boot script saved\n",
          __FUNCTION__)
         );
  return;
