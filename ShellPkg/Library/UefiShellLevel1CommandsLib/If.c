  ...
  switch (BinOp) {
    ...
    case OperatorGreaterThan:
      if (ForceStringCompare ||
          !ShellIsHexOrDecimalNumber (Compare1, FALSE, FALSE) || !ShellIsHexOrDecimalNumber (Compare2, FALSE, FALSE)) {
        //
        // string compare
        //
        if ((CaseInsensitive &&
             StringNoCaseCompare (&Compare1, &Compare2) > 0) || (StringCompare (&Compare1, &Compare2) > 0)) {
          return (TRUE);
        }
      } else {
        //
  ...
  )
{
  ...
  for (CommandNode = (SCRIPT_COMMAND_LIST *) GetNextNode (&ScriptFile->CommandList, &ScriptFile->CurrentCommand->Link),
       Found = FALSE
  ...
  for ( ShellStatus = SHELL_SUCCESS, CurrentValue = FALSE, Ending = EndTagMax
    ...
    if (gUnicodeCollation->StriColl (
      ...
      if (!BuildNextStatement (CurrentParameter, &EndParameter, &Ending)) {
        ...
        if (EFI_ERROR (Status)) {
          // ShellPrintHiiEx(-1, -1, NULL, STRING_TOKEN (STR_SYNTAX_STARTING), gShellLevel1HiiHandle,
          // gEfiShellParametersProtocol->Argv[CurrentParameter]);
          ShellStatus = SHELL_INVALID_PARAMETER;
        } else {
          //
