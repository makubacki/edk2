  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    OriginalOptionDataSize = sizeof (UINT32) + sizeof (UINT16) +
  ...
  )
{
  ...
  if (EFI_ERROR (Status) || (((UINT16) Intermediate) != Intermediate) ||
      StrStr (Walker, L" ") == NULL || ((UINT16) Intermediate) > ((UINT16) OrderCount)) {
  ...
  if (Walker[0] == L'\"') {
    ...
    if (StrStr (Walker, L" ") == NULL) {
      ...
      for (LoopCounter = 0;
           ShellStatus == SHELL_SUCCESS && LoopCounter < NewKeyOption.KeyData.Options.InputKeyCount;
           LoopCounter++) {
        //
        // ScanCode
        //
        ...
        ((EFI_INPUT_KEY *) (((UINT8 *) KeyOptionBuffer) +
                            sizeof (EFI_KEY_OPTION)))[LoopCounter].ScanCode = (UINT16) Intermediate;
        Temp = StrStr (Walker, L" ");
        if (Temp != NULL) {
          Walker = Temp;
        }
        ...
        ((EFI_INPUT_KEY *) (((UINT8 *) KeyOptionBuffer) +
                            sizeof (EFI_KEY_OPTION)))[LoopCounter].UnicodeChar = (UINT16) Intermediate;
        Temp = StrStr (Walker, L" ");
        if (Temp != NULL) {
          Walker = Temp;
        }
  ...
  if (EFI_ERROR (Status)) {
    ...
    if ((ShellCommandLineGetCount (Package) < 3 && CurrentOperation.Type != BcfgTypeOpt) ||
      ...
      ShellStatus = SHELL_INVALID_PARAMETER;
                                          ...
                                          gUnicodeCollation,
                                          (CHAR16 *) ShellCommandLineGetRawValue (Package, 1),
                                          L"driver"
                                          ) == 0) {
    ...
    if (ShellStatus == SHELL_SUCCESS && CurrentOperation.Target < BcfgTargetMax) {
      for (ParamNumber = 2;
           ParamNumber < ShellCommandLineGetCount (Package) && ShellStatus == SHELL_SUCCESS;
           ParamNumber++) {
        ...
        if        (gUnicodeCollation->StriColl (gUnicodeCollation, (CHAR16 *) CurrentParam, L"dump") == 0) {
          ...
          if (CurrentParam == NULL || !ShellIsHexOrDecimalNumber (CurrentParam, TRUE, FALSE)) {
            ...
            CurrentOperation.FileName =
              StrnCatGrow (&CurrentOperation.FileName, NULL, ShellCommandLineGetRawValue (Package, ++ParamNumber), 0);
            ASSERT (CurrentOperation.Description == NULL);
            CurrentOperation.Description = StrnCatGrow (
                                                      &CurrentOperation.Description,
                                                      NULL,
                                                      ShellCommandLineGetRawValue (Package, ++ParamNumber),
                                                      0
                                                      );
          }
          ...
          if ((ParamNumber + 2) >= ShellCommandLineGetCount (Package)) {
            ...
            if (CurrentParam == NULL || !ShellIsHexOrDecimalNumber (CurrentParam, TRUE, FALSE)) {
              ...
              if (CurrentOperation.Number1 >= Count) {
                ...
                ASSERT (CurrentOperation.FileName == NULL);
                CurrentOperation.FileName =
                  StrnCatGrow (&CurrentOperation.FileName, NULL, ShellCommandLineGetRawValue (Package, ++ParamNumber),
                               0);
              }
            }
          }
