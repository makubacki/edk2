  ...
  )
{
  ...
  for (Index = 0; Index < Size; Index++) {
    ShellPrintHiiEx (
      ...
      NULL,
      STRING_TOKEN (
        STR_IFCONFIG_INFO_MAC_ADDR_BODY
        ),
  ...
  )
{
  ...
  if (IsListEmpty (IfList)) {
    ShellPrintHiiEx (
      ...
      NULL,
      STRING_TOKEN (
        STR_IFCONFIG_INVALID_INTERFACE
        ),
      gShellNetwork1HiiHandle
      );
  }
  ...
  NET_LIST_FOR_EACH_SAFE (Entry, Next, IfList) {
    ...
    // Get Media State.
    //
    if (EFI_SUCCESS == NetLibDetectMediaWaitTimeout (
                         IfCb->NicHandle,
                         0,
                         &MediaStatus
                         ))
    ...
    ShellPrintHiiEx (
      ...
      NULL,
      STRING_TOKEN (
        STR_IFCONFIG_INFO_MAC_ADDR_HEAD
        ),
      gShellNetwork1HiiHandle
      );
    ...
    ShellPrintHiiEx (
      ...
      NULL,
      STRING_TOKEN (
        STR_IFCONFIG_INFO_IP_ADDR_HEAD
        ),
      gShellNetwork1HiiHandle
      );
    ...
    ShellPrintHiiEx (
      ...
      NULL,
      STRING_TOKEN (
        STR_IFCONFIG_INFO_SUBNET_MASK_HEAD
        ),
      gShellNetwork1HiiHandle
      );
    ...
    ShellPrintHiiEx (
      ...
      NULL,
      STRING_TOKEN (
        STR_IFCONFIG_INFO_GATEWAY_HEAD
        ),
      gShellNetwork1HiiHandle
      );
    ...
    for (Index = 0; Index < IfCb->IfInfo->RouteTableSize; Index++) {
      if ((CompareMem (
             &IfCb->IfInfo->RouteTable[Index].SubnetAddress,
             &mZeroIp4Addr,
             sizeof (EFI_IPv4_ADDRESS)
             ) == 0) &&
          (CompareMem (
             &IfCb->IfInfo->RouteTable[Index].SubnetMask,
             &mZeroIp4Addr,
             sizeof (EFI_IPv4_ADDRESS)
             ) == 0))
      {
        CopyMem (
          &Gateway,
          &IfCb->IfInfo->RouteTable[Index].GatewayAddress,
          sizeof (EFI_IPv4_ADDRESS)
          );
      }
    }
    ...
    for (Index = 0; Index < IfCb->IfInfo->RouteTableSize; Index++) {
      ShellPrintHiiEx (
        ...
        NULL,
        STRING_TOKEN (
          STR_IFCONFIG_ROUTES_ENTRY_INDEX
          ),
        gShellNetwork1HiiHandle,
        Index
        );
    ...
    ShellPrintHiiEx (
      ...
      NULL,
      STRING_TOKEN (
        STR_IFCONFIG_INFO_DNS_ADDR_HEAD
        ),
      gShellNetwork1HiiHandle
      );
      ...
      switch (CheckCode) {
        case VarCheckDuplicate:
          ShellPrintHiiEx (
            ...
            NULL,
            STRING_TOKEN (
              STR_IFCONFIG_DUPLICATE_COMMAND
              ),
            gShellNetwork1HiiHandle,
            VarArg->Arg
            );
        ...
        case VarCheckUnknown:
          ...
          if (StringNoCaseCompare (&VarArg->Arg, &TempString) == 0) {
            ShellPrintHiiEx (
              ...
              NULL,
              STRING_TOKEN (
                STR_IFCONFIG_UNSUPPORTED_OPTION
                ),
              gShellNetwork1HiiHandle,
              PermanentString
              );
    ...
    if (StrCmp (VarArg->Arg, L"dhcp") == 0) {
      ...
      if (EFI_ERROR (Status)) {
        ShellPrintHiiEx (
          ...
          NULL,
          STRING_TOKEN (
            STR_IFCONFIG_INVALID_IPADDRESS
            ),
          gShellNetwork1HiiHandle,
          VarArg->Arg
          );
  ...
  )
{
  ...
  switch (Private->OpCode) {
    ...
    case IfConfigOpClear:
      ShellStatus = IfConfigClearInterfaceInfo (
                      &Private->IfList,
                      Private->IfName
                      );
      break;
    ...
    case IfConfigOpSet:
      ShellStatus = IfConfigSetInterfaceInfo (
                      &Private->IfList,
                      Private->VarArg
                      );
      break;
