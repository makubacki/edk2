  ...
  )
{
  ...
  if (ShellIsDirectory (Source) == EFI_SUCCESS) {
    ...
    //
    // if the destination file already exists then it will be replaced, meaning the sourcefile effectively needs less
    // storage space
    //
    ...
    switch (ParamCount = ShellCommandLineGetCount (Package)) {
      ...
      default:
        ...
        for (ParamCount--, LoopCounter = 1; LoopCounter < ParamCount && ShellStatus == SHELL_SUCCESS; LoopCounter++) {
          ...
          Status = ShellOpenFileMetaArg (
                                       (CHAR16 *) ShellCommandLineGetRawValue (Package, LoopCounter),
                                       EFI_FILE_MODE_WRITE|EFI_FILE_MODE_READ,
                                       &FileList
                                       );
          if (EFI_ERROR (Status) || FileList == NULL || IsListEmpty (&FileList->Link)) {
            ShellPrintHiiEx (
                           ...
                           gShellLevel2HiiHandle,
                           L"cp",
                           ShellCommandLineGetRawValue (Package, LoopCounter)
                           );
            ShellStatus = SHELL_NOT_FOUND;
          }
        }
        ...
        if (ShellStatus != SHELL_SUCCESS) {
          ...
          if (FileList != NULL && !IsListEmpty (&FileList->Link)) {
            ShellStatus =
              ProcessValidateAndCopyFiles (
                                                         FileList,
                                                         PathCleanUpDirectories ((CHAR16 *) ShellCommandLineGetRawValue (
                                                                                                                           Package,
                                                                                                                           ParamCount)),
                                                         SilentMode,
                                                         RecursiveMode
