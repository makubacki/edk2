  ...
  )
{
  ...
  ShellStatus =
    GetDestinationLocation (
                                        ...
                                        &DestPath,
                                        FullCwd,
                                        (BOOLEAN) (FileList->Link.ForwardLink == FileList->Link.BackLink),
                                        &Attr
                                        );
  ...
  for (Node = (EFI_SHELL_FILE_INFO *) GetFirstNode (&FileList->Link)
    ...
    if (!IsValidMove (
                    Node->FullName,
                    FullCwd,
                    FullDestPath != NULL ? FullDestPath : DestPath,
                    Node->Info->Attribute,
                    Attr,
                    Node->Status
                    )) {
    ...
    if (IsBetweenFileSystem (Node->FullName, FullCwd, DestPath)) {
      while (FullDestPath == NULL && DestPath != NULL && DestPath[0] != CHAR_NULL &&
             DestPath[StrLen (DestPath) - 1] == L'\\') {
