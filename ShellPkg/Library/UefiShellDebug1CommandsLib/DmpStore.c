  ...
  )
{
  ...
  while (Position < FileSize) {
    ...
    *((UINT32 *) Buffer + 1) = DataSize;
    ...
    CopyMem (
            &Variable->Attributes,
            Buffer + sizeof (NameSize) + sizeof (DataSize) + NameSize + sizeof (EFI_GUID),
            sizeof (UINT32)
            );
    CopyMem (
            Variable->Data,
            Buffer + sizeof (NameSize) + sizeof (DataSize) + NameSize + sizeof (EFI_GUID) + sizeof (UINT32),
            DataSize
            );
  ...
  )
{
  ...
  *(UINT32 *) Ptr = Attributes;
  ...
  if (!EFI_ERROR (Status) &&
      (BufferSize !=
       sizeof (NameSize) + sizeof (DataSize) + sizeof (*Guid) + sizeof (Attributes) + NameSize + DataSize +
       sizeof (UINT32))
      ) {
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    ...
    if (ShellCommandLineGetCount (Package) > 2) {
      ...
      ShellStatus = SHELL_INVALID_PARAMETER;
    } else if ((ShellCommandLineGetFlag (
                                       Package,
                                       L"-s"
                                       ) ||
                ShellCommandLineGetFlag (Package, L"-l")) && ShellCommandLineGetFlag (Package, L"-d")) {
      ShellPrintHiiEx (
                     - 1,
                     - 1,
                     NULL,
      ...
      ShellStatus = SHELL_INVALID_PARAMETER;
                ...
                ShellCommandLineGetFlag (Package, L"-l")) && ShellCommandLineGetFlag (Package, L"-sfo")) {
      ShellPrintHiiEx (
                     - 1,
                     - 1,
                     NULL,
