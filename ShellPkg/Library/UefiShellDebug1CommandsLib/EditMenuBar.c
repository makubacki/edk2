  ...
  )
{
  ...
  for (NumItems = 0, ItemsWalker = Items;
       ItemsWalker != NULL && ItemsWalker->Function != NULL;
       ItemsWalker++, NumItems++) {
  }
  ...
  if (((KeyData->KeyState.KeyShiftState & EFI_SHIFT_STATE_VALID) == 0) ||
    ...
    ControlIndex = KeyData->Key.UnicodeChar;
             ...
             ((KeyData->KeyState.KeyShiftState &
               ~(EFI_SHIFT_STATE_VALID | EFI_RIGHT_CONTROL_PRESSED | EFI_LEFT_CONTROL_PRESSED)) == 0)) {
    //
