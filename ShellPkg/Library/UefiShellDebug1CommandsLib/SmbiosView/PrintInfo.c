...
#define AE_SMBIOS_VERSION(MajorVersion, MinorVersion) \
  (SmbiosMajorVersion > (MajorVersion) || \
   (SmbiosMajorVersion == (MajorVersion) && SmbiosMinorVersion >= (MinorVersion)))
  ...
  switch (Struct->Hdr->Type) {
    ...
    case 3:
      ...
      if (AE_SMBIOS_VERSION (0x2, 0x7) && (Struct->Hdr->Length > 0x13)) {
        if (Struct->Hdr->Length >
            (0x15 + (Struct->Type3->ContainedElementCount * Struct->Type3->ContainedElementRecordLength))) {
          PRINT_SMBIOS_STRING (
            Struct,
            Buffer[0x15 + (Struct->Type3->ContainedElementCount * Struct->Type3->ContainedElementRecordLength)],
            SKUNumber
            );
        }
      }
    ...
    case 10:
    {
      ...
      for (Index = 0; Index < NumOfDevice; Index++) {
        ShellPrintEx (
          - 1,
          - 1,
          (((Struct->Type10->Device[Index].DeviceType) & 0x80) != 0) ? L"Device Enabled\n" : L"Device Disabled\n"
          );
    ...
    case 41:
      ...
      ShellPrintEx (
        - 1,
        - 1,
        (((Struct->Type41->DeviceType) & 0x80) != 0) ? L"Device Enabled\n" : L"Device Disabled\n"
        );
