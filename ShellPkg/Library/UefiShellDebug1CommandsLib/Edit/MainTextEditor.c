      ...
      if (KeyData.Key.UnicodeChar == L'W' - L'A' + 1) {
        break;
               ...
               ((KeyData.KeyState.KeyShiftState &
                 ~(EFI_SHIFT_STATE_VALID | EFI_LEFT_CONTROL_PRESSED | EFI_RIGHT_CONTROL_PRESSED)) == 0)) {
      //
  ...
  )
{
  ...
  Gap = (MainEditor.MouseAccumulatorX * (INT32) MainEditor.ScreenSize.Column) /
        (INT32) (50 * (INT32) MainEditor.MouseInterface->Mode->ResolutionX);
  ...
  )
{
  ...
  Gap = (MainEditor.MouseAccumulatorY * (INT32) MainEditor.ScreenSize.Row) /
        (INT32) (50 * (INT32) MainEditor.MouseInterface->Mode->ResolutionY);
    ...
    if (!EFI_ERROR (Status)) {
      ...
      if (!EFI_ERROR (Status)) {
        ...
        if (EFI_NOT_FOUND != MenuBarDispatchControlHotKey (&KeyData)) {
          Status = EFI_SUCCESS;
        } else if (NoShiftState &&
                   ((KeyData.Key.ScanCode == SCAN_NULL) ||
                    ((KeyData.Key.ScanCode >= SCAN_UP) && (KeyData.Key.ScanCode <= SCAN_PAGE_DOWN)))) {
