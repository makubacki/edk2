  ...
  )
{
  ...
  if (gEfiShellProtocol != NULL) {
    ...
    for ( Node = GetFirstNode (&(*ListHead)->Link)
      ...
      ((EFI_FILE_PROTOCOL *)((EFI_SHELL_FILE_INFO_NO_CONST *)Node)->Handle)->Close (
                                                                               ((EFI_SHELL_FILE_INFO_NO_CONST *)
                                                                                Node)->Handle
                                                                               );
  ...
  )
{
  ...
  if (((Name[0] == L'-' || Name[0] == L'+') &&
       InternalShellIsHexOrDecimalNumber (Name+1, FALSE, FALSE, TimeNumbers)) && AlwaysAllowNumbers) {
    return (FALSE);
  }
      ...
      switch (CurrentItemPackage->Type) {
        ...
        default:
          ...
          break;
               ...
               !InternalIsFlag (Argv[LoopCounter], AlwaysAllowNumbers,
  ...
  )
{
  ...
  for ( Node1 = GetFirstNode (CheckPackage)
    ...
    for ( Node2 = GetNextNode (CheckPackage, Node1)
      ...
      if ((((SHELL_PARAM_PACKAGE *)Node1)->Name != NULL) && (((SHELL_PARAM_PACKAGE *)Node2)->Name != NULL) &&
          StrCmp (((SHELL_PARAM_PACKAGE *)Node1)->Name, ((SHELL_PARAM_PACKAGE *)Node2)->Name) == 0) {
        if (Param != NULL) {
          *Param = NULL;
        ...
        switch (*(ResumeLocation+1)) {
          ...
          case (L'E'):
            gST->ConOut->SetAttribute (
                           gST->ConOut,
                           EFI_TEXT_ATTR (EFI_YELLOW, ((OriginalAttribute&(BIT4|BIT5|BIT6))>>4))
                           );
            break;
          case (L'H'):
            gST->ConOut->SetAttribute (
          ...
          case (L'V'):
            gST->ConOut->SetAttribute (
                           gST->ConOut,
                           EFI_TEXT_ATTR (EFI_LIGHTGREEN, ((OriginalAttribute&(BIT4|BIT5|BIT6))>>4))
                           );
            break;
          default:
            //
