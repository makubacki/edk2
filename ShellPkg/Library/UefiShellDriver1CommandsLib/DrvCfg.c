...
STATIC CONST EFI_GUID  *CfgGuidList[] = {
  &gEfiDriverConfigurationProtocolGuid, &gEfiDriverConfiguration2ProtocolGuid, NULL
};
  ...
  )
{
  ...
  for (LoopVariable = 0;
       LoopVariable < (HandleBufferSize/sizeof (HandleBuffer[0])) && *HiiHandle == NULL;
       LoopVariable++) {
    MainBufferSize = 0;
    MainBuffer     = NULL;
    ...
    for (PackageListHeader = (EFI_HII_PACKAGE_LIST_HEADER *) MainBuffer
         ...
         *HiiHandle == NULL
         ;
         PackageListHeader =
           (EFI_HII_PACKAGE_LIST_HEADER *) (((CHAR8 *) (PackageListHeader)) + PackageListHeader->PackageLength)) {
      for (PackageHeader =
             (EFI_HII_PACKAGE_HEADER *) (((CHAR8 *) (PackageListHeader))+sizeof (EFI_HII_PACKAGE_LIST_HEADER))
           ;
           PackageHeader != NULL && ((CHAR8 *) PackageHeader) < (((CHAR8 *) MainBuffer)+MainBufferSize) &&
           PackageHeader->Type != EFI_HII_PACKAGE_END && *HiiHandle == NULL
    ...
    for (PackageListHeader = (EFI_HII_PACKAGE_LIST_HEADER *) MainBuffer
      ...
      for (PackageHeader =
        ...
        if (PackageHeader->Type == EFI_HII_PACKAGE_DEVICE_PATH) {
          HiiHandle = NULL;
          Status    =
            FindHiiHandleViaDevPath ((EFI_DEVICE_PATH_PROTOCOL *) (((CHAR8 *) PackageHeader) +
          ...
          if (EFI_ERROR (Status)) {
            ...
            //
            TempDevPathString =
              ConvertDevicePathToText ((EFI_DEVICE_PATH_PROTOCOL *) (((CHAR8 *) PackageHeader) +
                                                                     sizeof (EFI_HII_PACKAGE_HEADER)), TRUE, TRUE);
            ShellPrintHiiEx (
                             - 1,
                             - 1,
                             NULL,
  ...
  )
{
  ...
  switch (ActionRequired) {
    ...
    case EfiDriverConfigurationActionRestartController:
      ...
      ShellPrintHiiEx (- 1, - 1, NULL, STRING_TOKEN (STR_DRVCFG_ENTER_S), gShellDriver1HiiHandle,
  ...
  )
{
  ...
  if (EFI_ERROR (Status) || ShellCommandLineGetCount (Package) > 2) {
    ...
    if (EFI_ERROR (Status)) {
      if (Status == EFI_VOLUME_CORRUPTED && ProblemParam != NULL) {
        ShellPrintHiiEx (- 1, - 1, NULL, STRING_TOKEN (STR_GEN_PROBLEM), gShellDriver1HiiHandle, L"drvcfg",
                         ProblemParam);
  ...
  if (ShellStatus == SHELL_SUCCESS) {
    ...
    if (UseHii) {
      if (Handle1 != NULL &&
          EFI_ERROR (gBS->OpenProtocol (Handle1, &gEfiHiiConfigAccessProtocolGuid, NULL, gImageHandle, NULL,
        ...
        if (!EFI_ERROR (gBS->OpenProtocol (Handle1, &gEfiHiiConfigAccessProtocolGuid, NULL, gImageHandle, NULL,
                                           EFI_OPEN_PROTOCOL_TEST_PROTOCOL))) {
          ShellPrintHiiEx (
                           - 1,
                           - 1,
