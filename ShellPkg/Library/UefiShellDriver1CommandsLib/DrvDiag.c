  ...
  for (DriverHandleListLoop = 0
    ...
    if (Mode == TestModeList) {
      ShellPrintHiiEx (- 1,
                       ...
                       gShellDriver1HiiHandle,
                       ConvertHandleToHandleIndex (DriverHandleList[DriverHandleListLoop]));
    }
        ...
        for (ChildHandleListLoop = 0
          ...
          if (Mode != TestModeList) {
            if (Lang == NULL || Lang[2] == '-') {
              ...
              if (!EFI_ERROR (Status) && (DriverDiagnostics2 != NULL)) {
                ...
                Status   = DriverDiagnostics2->RunDiagnostics (
                                                               ...
                                                               ControllerHandleList[ControllerHandleListLoop],
                                                               ChildHandleList ==
                                                               NULL ? NULL : ChildHandleList[ChildHandleListLoop],
            ...
            ShellPrintHiiEx (
                             ...
                             HandleIndex2,
                             ChildHandleList ==
                             NULL ? 0 : ConvertHandleToHandleIndex (ChildHandleList[ChildHandleListLoop]),
                             Status
                             );
            if (OutBuffer != NULL) {
              FreePool (OutBuffer);
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    if (Status == EFI_VOLUME_CORRUPTED && ProblemParam != NULL) {
      ShellPrintHiiEx (- 1, - 1, NULL, STRING_TOKEN (STR_GEN_PROBLEM), gShellDriver1HiiHandle, L"drvdiag",
                       ProblemParam);
