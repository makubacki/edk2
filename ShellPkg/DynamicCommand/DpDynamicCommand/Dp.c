  ...
  switch (RecordHeader->Type) {
    ...
    case FPDT_DYNAMIC_STRING_EVENT_TYPE:
      ...
      if (Measurement->Token != NULL && AsciiStrCmp (Measurement->Token, ALit_PEIM) == 0) {
        ...
        //
        if (Measurement->Token == NULL  &&
            (Measurement->Identifier == MODULE_START_ID || Measurement->Identifier == MODULE_END_ID)) {
          if (StartHandle == NULL && !IsZeroGuid (ModuleGuid)) {
            Measurement->Token  = ALit_PEIM;
            Measurement->Handle = ModuleGuid;
          } else {
  ...
  )
{
  ...
  for (Index = mMeasurementNum - 1; Index >= 0; Index--) {
    if (AsciiStrCmp (EndMeasureMent->Token, ALit_PEIM) == 0) {
      if (mMeasurementList[Index].EndTimeStamp == 0 && EndMeasureMent->Handle != NULL &&
          mMeasurementList[Index].Handle != NULL &&
  ...
  )
{
  ...
  while (TableLength < mBootPerformanceTableSize) {
    ...
    //
    // If the record with ProgressId 0, the record doesn't appear in pairs. The timestamp in the record is the
    // EndTimeStamp, its StartTimeStamp is 0.
    // If the record is the start record, fill the info to the measurement in the mMeasurementList.
    // If the record is the end record, find the related start measurement in the mMeasurementList and fill the
    // EndTimeStamp.
    //
  ...
  )
{
  ...
  SummaryMode =
    (BOOLEAN) (ShellCommandLineGetFlag (ParamPackage, L"-S") || ShellCommandLineGetFlag (ParamPackage, L"-s"));
  ...
  if (VerboseMode && !RawMode) {
    StringPtr = HiiGetString (
                              mDpHiiHandle,
                              (EFI_STRING_ID) (TimerInfo.CountUp ? STRING_TOKEN (STR_DP_UP) : STRING_TOKEN (
                                                                                               STR_DP_DOWN
                                                                                               )),
                              NULL
                              );
