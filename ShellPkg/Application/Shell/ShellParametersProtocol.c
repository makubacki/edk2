  ...
  )
{
  ...
  if (ShellParameters == NULL || SystemTableInfo == NULL || OldStdIn == NULL || OldStdOut == NULL ||
      OldStdErr == NULL) {
    return (EFI_INVALID_PARAMETER);
  }
  ...
  if (!EFI_ERROR (Status)) {
    ...
    if (
       ...
       || (StdOutFileName != NULL && OutUnicode && OutAppend &&
           (!EFI_ERROR (ShellFileExists (StdOutFileName)) && EFI_ERROR (IsUnicodeFile (StdOutFileName))))
       || (StdErrFileName != NULL && ErrUnicode && ErrAppend &&
           (!EFI_ERROR (ShellFileExists (StdErrFileName)) && EFI_ERROR (IsUnicodeFile (StdErrFileName))))
       || (StdOutFileName != NULL && !OutUnicode && OutAppend &&
           (!EFI_ERROR (ShellFileExists (StdOutFileName)) && !EFI_ERROR (IsUnicodeFile (StdOutFileName))))
       || (StdErrFileName != NULL && !ErrUnicode && ErrAppend &&
           (!EFI_ERROR (ShellFileExists (StdErrFileName)) && !EFI_ERROR (IsUnicodeFile (StdErrFileName))))
      ...
      if (!EFI_ERROR (Status) && StdInVarName != NULL) {
        ...
        if (TempHandle == NULL) {
          ...
          Size = 0;
          if (TempHandle == NULL ||
              ((EFI_FILE_PROTOCOL *) TempHandle)->Read (TempHandle, &Size, NULL) != EFI_BUFFER_TOO_SMALL) {
            Status = EFI_INVALID_PARAMETER;
          } else {
