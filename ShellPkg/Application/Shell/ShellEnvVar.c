    ...
    if (!EFI_ERROR (Status) && CompareGuid (&Guid, &gShellVariableGuid)) {
      ...
      if (VarList == NULL) {
        ...
        if (Status == EFI_BUFFER_TOO_SMALL) {
          ...
          ValSize = ValBufferSize;
          Status  =
            SHELL_GET_ENVIRONMENT_VARIABLE_AND_ATTRIBUTES (VariableName, &VarList->Atts, &ValSize, VarList->Val);
        }
  ...
  )
{
  ...
  for ( CurrentCount = 0
    ...
    //
    // ValueSize = TotalSize - already removed size - size for '=' + size for terminator (the last 2 items cancel each
    // other)
    //
    Node->Val =
      AllocateCopyPool (StrSize (CurrentString) - StrSize (Node->Key), CurrentString + StrLen (Node->Key) + 1);
    if (Node->Val == NULL) {
