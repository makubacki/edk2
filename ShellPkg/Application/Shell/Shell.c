  ...
  )
{
  ...
  while ((StrLen (*String) > 0) &&
         (((*String)[StrLen ((*String))-1] == L' ') || ((*String)[StrLen ((*String))-1] == L'\t'))) {
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      if (!ShellInfoObject.ShellInitSettings.BitUnion.Bits.Exit && !ShellCommandGetExit () &&
          (PcdGet8 (PcdShellSupportLevel) >= 3 || PcdGetBool (PcdShellForceConsole)) && !EFI_ERROR (Status) &&
  ...
  for (LoopVar = 0; LoopVar < gEfiShellParametersProtocol->Argc; LoopVar++) {
    ...
    if (UnicodeCollation->StriColl (
      ...
      for (Size = 0; LoopVar < gEfiShellParametersProtocol->Argc; LoopVar++) {
        ASSERT (
              (ShellInfoObject.ShellInitSettings.FileOptions == NULL && Size == 0) ||
  ...
  if (NewScriptFile->Argc != 0) {
    ...
    for (LoopVar = 1; LoopVar < 10 && LoopVar < NewScriptFile->Argc; LoopVar++) {
      ...
      NewScriptFile->Argv[LoopVar] = StrnCatGrow (
                                                ...
                                                NULL,
                                                ShellInfoObject.NewShellParametersProtocol->Argv[LoopVar],
                                                0
                                                );
