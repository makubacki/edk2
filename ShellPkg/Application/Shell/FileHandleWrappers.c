        ...
        switch (Key.ScanCode) {
          ...
          case SCAN_DOWN:
            ...
            if (NewPos == (BUFFER_LIST *)(&ShellInfoObject.ViewingSettings.CommandHistory)) {
              NewPos =
                (BUFFER_LIST *)GetNextNode (&ShellInfoObject.ViewingSettings.CommandHistory.Link, &LinePos->Link);
            }
  ...
  if (MemFile->Unicode) {
    ...
    if ((UINTN)(MemFile->Position + AsciiStrSize (AsciiBuffer)) > (UINTN)(MemFile->BufferSize)) {
      MemFile->Buffer =
        ReallocatePool (
          (UINTN)(MemFile->BufferSize),
          (UINTN)(MemFile->BufferSize) + AsciiStrSize (AsciiBuffer) + MEM_WRITE_REALLOC_OVERHEAD,
          MemFile->Buffer
          );
      if (MemFile->Buffer == NULL) {
  ...
  )
{
  ...
  if (((EFI_FILE_PROTOCOL_FILE *)This)->Unicode) {
    ...
    *BufferSize = *BufferSize / sizeof (CHAR16) * sizeof (CHAR16);
    ...
    Status =
      ((EFI_FILE_PROTOCOL_FILE *)This)->Orig->Read (((EFI_FILE_PROTOCOL_FILE *)This)->Orig, &Size, AsciiStrBuffer);
    if (!EFI_ERROR (Status)) {
  ...
  )
{
  ...
  if (((EFI_FILE_PROTOCOL_FILE *)This)->Unicode) {
    ...
    Status =
      (((EFI_FILE_PROTOCOL_FILE *)This)->Orig->Write (((EFI_FILE_PROTOCOL_FILE *)This)->Orig, &Size, AsciiBuffer));
    FreePool (AsciiBuffer);
    return (Status);
  }
}
