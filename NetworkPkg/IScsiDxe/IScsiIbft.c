  ...
  )
{
  ...
  Control->Header.Length =
    (UINT16)sizeof (EFI_ACPI_ISCSI_BOOT_FIRMWARE_TABLE_CONTROL_STRUCTURE);
  ...
  if (NumOffset > 4) {
    ...
    // exist.
    //
    Control->Header.Length = (UINT16)(Control->Header.Length + (NumOffset - 4) *
                                      sizeof (UINT16));
  }
}
  ...
  )
{
  ...
  Initiator->Header.Length =
    (UINT16)sizeof (EFI_ACPI_ISCSI_BOOT_FIRMWARE_TABLE_INITIATOR_STRUCTURE);
  ...
  IScsiAddHeapItem (
    ...
    mPrivate->InitiatorNameLength - 1
    );
  ...
  )
{
  ...
  NET_LIST_FOR_EACH (Entry, &mPrivate->AttemptConfigs) {
    if (Index == 0) {
      ...
      //
      Attempt = IScsiConfigGetAttemptByConfigIndex (
                  mPrivate->BootSelectedIndex
                  );
      if (Attempt == NULL) {
        //
    ...
    //
    if (mPrivate->EnableMpio && (Attempt->SessionConfigData.Enabled !=
    ...
    Nic->Header.Length =
      (UINT16)sizeof (EFI_ACPI_ISCSI_BOOT_FIRMWARE_TABLE_NIC_STRUCTURE);
    ...
    if ((NvData->IpMode == IP_MODE_IP4) || (NvData->IpMode ==
                                            IP_MODE_AUTOCONFIG))
    {
      //
    ...
    } else if ((NvData->IpMode == IP_MODE_IP6) || (NvData->IpMode ==
                                                   IP_MODE_AUTOCONFIG))
    {
    ...
    *SectionOffset = (UINT16)((UINTN)Nic - (UINTN)Table);
    ...
    Target->Header.Length =
      (UINT16)sizeof (EFI_ACPI_ISCSI_BOOT_FIRMWARE_TABLE_TARGET_STRUCTURE);
  ...
  )
{
  ...
  if ((!mPrivate->EnableMpio && (mPrivate->ValidSinglePathCount == 0)) ||
      (mPrivate->EnableMpio && (mPrivate->MpioCount <=
