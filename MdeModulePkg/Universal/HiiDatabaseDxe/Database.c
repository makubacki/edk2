  ...
  )
{
  ...
  while (VariableHeader < VariableEnd) {
    ...
    VariableHeader =
      (VARIABLE_HEADER *)((UINT8 *)VariableHeader + sizeof (VARIABLE_HEADER) + VariableHeader->NameSize +
  ...
  )
{
  ...
  if (Link == &gVarStorageList) {
    ...
    NvStoreBuffer =
      (VARIABLE_STORE_HEADER *)((UINT8 *)DataHeader + sizeof (DataHeader->DataSize) + DataHeader->HeaderSize);
  ...
  if (ValueBuffer != NULL) {
    if (BitFieldQuestion) {
      CopyMem (
        &BufferValue,
        (UINT8 *)VariableHeader + sizeof (VARIABLE_HEADER) + VariableHeader->NameSize + ByteOffset,
        Width
        );
      ...
      CopyMem (
        ValueBuffer,
        (UINT8 *)VariableHeader + sizeof (VARIABLE_HEADER) + VariableHeader->NameSize + IfrQuestionHdr->VarStoreInfo.VarOffset,
        Width
        );
    }
  }
      ...
      case EFI_IFR_VARSTORE_EFI_OP:
        if (EfiVarStoreNumber >= EfiVarStoreMaxNum) {
          ...
          EfiVarStoreList = ReallocatePool (
                              ...
                              (EfiVarStoreMaxNum + BASE_NUMBER) * sizeof (UINTN),
                              EfiVarStoreList
                              );
        ...
        EfiVarStoreList[EfiVarStoreNumber] =
          AllocatePool (IfrEfiVarStore->Header.Length + AsciiStrSize ((CHAR8 *)IfrEfiVarStore->Name));
  ...
  )
{
  ...
  StrCpyS (
    FontInfo->FontName,
    (FontInfoSize - OFFSET_OF (EFI_FONT_INFO, FontName)) / sizeof (CHAR16),
  ...
  )
{
  ...
  for (Link = PackageList->SimpleFontPkgHdr.ForwardLink;
       Link != &PackageList->SimpleFontPkgHdr;
       Link = Link->ForwardLink) {
  ...
  )
{
  ...
  for (Link = PackageList->KeyboardLayoutHdr.ForwardLink;
       Link != &PackageList->KeyboardLayoutHdr;
       Link = Link->ForwardLink) {
  ...
  )
{
  ...
  for (Link = Private->DatabaseList.ForwardLink; Link != &Private->DatabaseList; Link = Link->ForwardLink) {
    ...
    switch (PackageType) {
      case EFI_HII_PACKAGE_TYPE_GUID:
        for (Link1 = PackageList->GuidPkgHdr.ForwardLink; Link1 != &PackageList->GuidPkgHdr;
