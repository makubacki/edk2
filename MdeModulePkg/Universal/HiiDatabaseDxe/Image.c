  ...
  while (CurrentImageBlock->BlockType != EFI_HII_IIBT_END) {
    ...
    switch (CurrentImageBlock->BlockType) {
        ...
        Length = sizeof (EFI_HII_IIBT_IMAGE_24BIT_BLOCK) - sizeof (EFI_HII_RGB_PIXEL) +
                 BITMAP_LEN_24_BIT (
                   (UINT32)ReadUnaligned16 (
                             (VOID *)&((EFI_HII_IIBT_IMAGE_24BIT_BLOCK *)CurrentImageBlock)->Bitmap.Width
                             ),
      ...
      case EFI_HII_IIBT_IMAGE_JPEG:
        Length =
          OFFSET_OF (EFI_HII_IIBT_JPEG_BLOCK, Data) + ReadUnaligned32 (
                                                        (VOID *)&((EFI_HII_IIBT_JPEG_BLOCK *)CurrentImageBlock)->Size
                                                        );
        ImageIdCurrent++;
        break;
      ...
      case EFI_HII_IIBT_IMAGE_PNG:
        Length =
          OFFSET_OF (EFI_HII_IIBT_PNG_BLOCK, Data) + ReadUnaligned32 (
                                                       (VOID *)&((EFI_HII_IIBT_PNG_BLOCK *)CurrentImageBlock)->Size
                                                       );
        ImageIdCurrent++;
        break;
  ...
  if (PackageListNode->ImagePkg != NULL) {
    ...
    *ImageId = 0;
    ...
    ImageBlocks = (EFI_HII_IMAGE_BLOCK *)(
                                          (UINT8 *)ImageBlocks + ImagePackage->ImageBlockSize -
    ...
    *ImageId = 1;
    ...
    //
    ImagePackage->ImagePkgHdr.Header.Length = sizeof (EFI_HII_IMAGE_PACKAGE_HDR) + NewBlockSize +
                                              sizeof (EFI_HII_IIBT_END_BLOCK);
  ...
  switch (CurrentImageBlock->BlockType) {
    ...
    case EFI_HII_IIBT_IMAGE_24BIT_TRANS:
      OldBlockSize = sizeof (EFI_HII_IIBT_IMAGE_24BIT_BLOCK) - sizeof (EFI_HII_RGB_PIXEL) +
                     BITMAP_LEN_24_BIT (
                       (UINT32)ReadUnaligned16 (
                                 (VOID *)&((EFI_HII_IIBT_IMAGE_24BIT_BLOCK *)CurrentImageBlock)->Bitmap.
                                   Width
                                 ),
                       ReadUnaligned16 ((VOID *)&((EFI_HII_IIBT_IMAGE_24BIT_BLOCK *)CurrentImageBlock)->Bitmap.Height)
                       );
