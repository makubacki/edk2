...
Done:
  for (LinkData = VarStorageData->BlockEntry.ForwardLink;
       LinkData != &VarStorageData->BlockEntry;
       LinkData = LinkData->ForwardLink) {
  ...
  )
{
  ...
  for (FirstDefaultLink = FirstListEntry->ForwardLink;
       FirstDefaultLink != FirstListEntry;
       FirstDefaultLink = FirstDefaultLink->ForwardLink) {
    FirstDefaultValueData = BASE_CR (FirstDefaultLink, IFR_DEFAULT_DATA, Entry);
    SecondListEntry = &SecondBlock->DefaultValueEntry;
    for (SecondDefaultLink = SecondListEntry->ForwardLink;
         SecondDefaultLink != SecondListEntry;
         SecondDefaultLink = SecondDefaultLink->ForwardLink) {
      ...
      if (FirstBlock->Offset > SecondBlock->Offset) {
        ...
        *SecondDefaultValue = (UINT32)BitFieldRead64 (
                                        TotalValue,
                                        OffsetShift * 8,
                                        OffsetShift * 8 + SecondBlock->Width *8 -1
                                        );
      }
    }
  }
  ...
  )
{
  ...
  for (Link = BlockLink->ForwardLink; Link != BlockLink; Link = Link->ForwardLink) {
    ...
    for (TempLink = Link->ForwardLink; TempLink != BlockLink; TempLink = TempLink->ForwardLink) {
      ...
      if (!NextBlockData->IsBitVar || NextBlockData->Offset >= BlockData->Offset + BlockData->Width ||
          BlockData->Offset >= NextBlockData->Offset + NextBlockData->Width) {
        continue;
      }
  ...
  while (*StringPtr != 0 && StrnCmp (StringPtr, L"GUID=", StrLen (L"GUID=")) == 0) {
    ...
    *(ConfigRequest + Length) = 0;
    ...
    for (Link = Private->DatabaseList.ForwardLink;
      ...
      if ((DevicePathPkg = Database->PackageList->DevicePathPkg) != NULL) {
        ...
        if ((CompareMem (
               ...
               CurrentDevicePath,
               DevicePathSize
               ) == 0) && IsThisPackageList (Database, ConfigRequest)) {
    ...
    if ((HiiHandle != NULL) && !GetElementsFromRequest (ConfigRequest)) {
      ...
      Status =
        GetFullStringFromHiiFormPackages (Database, DevicePath, &ConfigRequest, &DefaultResults, &AccessProgress);
      if (EFI_ERROR (Status)) {
        //
    ...
    for (Link = Private->DatabaseList.ForwardLink;
      ...
      if ((DevicePathPkg = Database->PackageList->DevicePathPkg) != NULL) {
        ...
        if ((CompareMem (
               ...
               CurrentDevicePath,
               DevicePathSize
               ) == 0) && IsThisPackageList (Database, ConfigResp)) {
          DriverHandle = Database->DriverHandle;
          break;
        }
      }
