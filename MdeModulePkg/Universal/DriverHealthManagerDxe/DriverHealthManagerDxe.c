  ...
  )
{
  ...
  for (Offset = sizeof (EFI_HII_PACKAGE_LIST_HEADER);
       Offset < ReadUnaligned32 (&HiiPackageList->PackageLength);
  ...
  )
{
  ...
  for (Index = 0; Index < mDriverHealthManagerHealthInfoCount; Index++) {
    if (ConfigureOnly &&
        mDriverHealthManagerHealthInfo[Index].HealthStatus != EfiDriverHealthStatusConfigurationRequired) {
      continue;
    }
    ...
    if (mDriverHealthManagerHealthInfo[Index].MessageList != NULL) {
      for (MessageIndex = 0;
           mDriverHealthManagerHealthInfo[Index].MessageList[MessageIndex].HiiHandle != NULL;
           MessageIndex++) {
        ...
        StringCount += UnicodeSPrint (
                         String + StringCount,
                         sizeof (String) - sizeof (String[0]) * StringCount,
                         L"\n%s",
                         TmpString
                         );
        FreePool (TmpString);
