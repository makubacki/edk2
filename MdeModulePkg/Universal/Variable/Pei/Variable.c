  ...
  if (StoreInfo->FtwLastWriteData != NULL) {
    ...
    if (((UINTN)Name1 < (UINTN)TargetAddress) && (((UINTN)Name1 + NameSize) > (UINTN)TargetAddress)) {
      ...
      if (CompareMem ((UINT8 *)Name1, (UINT8 *)Name2, PartialNameSize) == 0) {
        ...
        //
        if (CompareMem (
              (UINT8 *)(UINTN)SpareAddress,
              (UINT8 *)Name2 + PartialNameSize,
              NameSize - PartialNameSize
              ) == 0) {
          return TRUE;
        }
      }
      ...
      if (!PcdGetBool (PcdEmuVariableNvModeEnable)) {
        ...
        if (GuidHob != NULL) {
          ...
          StoreInfo->IndexTable =
            (VARIABLE_INDEX_TABLE *)BuildGuidHob (&gEfiVariableIndexTableGuid, sizeof (VARIABLE_INDEX_TABLE));
  ...
  )
{
  ...
  *VariableHeader = Variable;
  ...
  if (StoreInfo->FtwLastWriteData != NULL) {
    ...
    if (((UINTN)Variable < (UINTN)TargetAddress) &&
        (((UINTN)Variable + GetVariableHeaderSize (StoreInfo->AuthFlag)) > (UINTN)TargetAddress)) {
      //
      ...
      if (GuidHob != NULL) {
        ...
        *VariableHeader =
          (VARIABLE_HEADER *)BuildGuidHob (&gEfiCallerIdGuid, GetVariableHeaderSize (StoreInfo->AuthFlag));
        ...
        //
        CopyMem (
          (UINT8 *)*VariableHeader + PartialHeaderSize,
          (UINT8 *)(UINTN)SpareAddress,
          GetVariableHeaderSize (StoreInfo->AuthFlag) - PartialHeaderSize
          );
      }
    }
  } else {
  ...
  while (TRUE) {
    ...
    if (VariableHeader->State == VAR_ADDED || VariableHeader->State == (VAR_IN_DELETED_TRANSITION & VAR_ADDED)) {
      ...
      if (VarNameSize <= *VariableNameSize) {
        GetVariableNameOrData (
          &StoreInfo,
          (UINT8 *)GetVariableNamePtr (Variable.CurrPtr, StoreInfo.AuthFlag),
          VarNameSize,
          (UINT8 *)VariableName
          );
