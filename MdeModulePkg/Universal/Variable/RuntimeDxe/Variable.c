  ...
  )
{
  ...
  while (IsValidVariableHeader (Variable, GetEndPointer (VariableStoreHeader))) {
    ...
    if (Variable != UpdatingVariable && Variable->State == VAR_ADDED) {
      ...
      if ((!IsVolatile) &&
          ((Variable->Attributes & EFI_VARIABLE_HARDWARE_ERROR_RECORD) == EFI_VARIABLE_HARDWARE_ERROR_RECORD)) {
  ...
  while (IsValidVariableHeader (Variable, GetEndPointer (VariableStoreHeader))) {
    NextVariable = GetNextVariablePtr (Variable, AuthFormat);
    if (Variable != UpdatingVariable && Variable != UpdatingInDeletedTransition &&
    ...
    if (UpdatingVariable != NULL) {
      UpdatingPtrTrack->CurrPtr =
        (VARIABLE_HEADER *)((UINTN)UpdatingPtrTrack->StartPtr +
                            ((UINTN)CurrPtr - (UINTN)GetStartPointer ((VARIABLE_STORE_HEADER *)ValidBuffer)));
  ...
  )
{
  ...
  if (Iso639Language) {
    ...
    for (Index = 0; *SupportedLang != '\0'; Index++, SupportedLang += CompareLength) {
      ...
      //
      for (CompareLength = 0;
           SupportedLang[CompareLength] != '\0' && SupportedLang[CompareLength] != ';';
           CompareLength++) {
      }
  ...
  )
{
  ...
  if ((CacheVariable->CurrPtr != NULL) &&
      ...
      (CacheVariable->StartPtr ==
       GetStartPointer ((VARIABLE_STORE_HEADER *)(UINTN)mVariableModuleGlobal->VariableGlobal.HobVariableBase))
      ) {
  ...
  if ((CacheVariable->CurrPtr == NULL) || CacheVariable->Volatile) {
    ...
    Variable->EndPtr   =
      (VARIABLE_HEADER *)((UINTN)Variable->StartPtr + ((UINTN)CacheVariable->EndPtr - (UINTN)CacheVariable->StartPtr));

    Variable->CurrPtr =
      (VARIABLE_HEADER *)((UINTN)Variable->StartPtr + ((UINTN)CacheVariable->CurrPtr - (UINTN)CacheVariable->StartPtr));
    if (CacheVariable->InDeletedTransitionPtr != NULL) {
      Variable->InDeletedTransitionPtr =
        (VARIABLE_HEADER *)((UINTN)Variable->StartPtr +
                            ((UINTN)CacheVariable->InDeletedTransitionPtr - (UINTN)CacheVariable->StartPtr));
    } else {
  ...
  //
  NextVariable =
    GetEndPointer ((VARIABLE_STORE_HEADER *)((UINTN)mVariableModuleGlobal->VariableGlobal.VolatileVariableBase));
  ...
  if ((Attributes & EFI_VARIABLE_NON_VOLATILE) != 0) {
    ...
    if (  (  ((Attributes & EFI_VARIABLE_HARDWARE_ERROR_RECORD) != 0)
       ...
       || (IsCommonVariable &&
           ((VarSize + mVariableModuleGlobal->CommonVariableTotalSize) > mVariableModuleGlobal->CommonVariableSpace))
       || (IsCommonVariable && AtRuntime () &&
      ...
      if (!EFI_ERROR (Status)) {
        ...
        if (Variable->CurrPtr != NULL) {
          CacheVariable->CurrPtr =
            (VARIABLE_HEADER *)((UINTN)CacheVariable->StartPtr +
                                ((UINTN)Variable->CurrPtr - (UINTN)Variable->StartPtr));
    ...
    if (!mVariableModuleGlobal->VariableGlobal.EmuNvMode) {
      ...
      //
      CopyMem (
        (UINT8 *)mNvVariableCache + mVariableModuleGlobal->NonVolatileLastVariableOffset,
        (UINT8 *)NextVariable,
        VarSize
        );
    } else {
  ...
  )
{
  ...
  VariableStoreHeader[VariableStoreTypeHob] =
    (VARIABLE_STORE_HEADER *)(UINTN)mVariableModuleGlobal->VariableGlobal.HobVariableBase;
  VariableStoreHeader[VariableStoreTypeNv] = mNvVariableCache;
  ...
  )
{
  ...
  Status =
    SetVariableCheckHandlerMor (
      ...
      Attributes,
      PayloadSize,
      (VOID *)((UINTN)Data + DataSize - PayloadSize)
      );
  ...
  )
{
  ...
  if (mVariableModuleGlobal->CommonRuntimeVariableSpace < mVariableModuleGlobal->CommonVariableTotalSize) {
    ...
    RemainingCommonRuntimeVariableSpace = mVariableModuleGlobal->CommonRuntimeVariableSpace -
                                          mVariableModuleGlobal->CommonVariableTotalSize;
  }
  ...
  )
{
  ...
  while (StartPtr < EndPtr) {
    if (StartPtr->State == VAR_ADDED) {
      ...
      CopyMem (
        NextPtr,
        GetVariableDataPtr (StartPtr, mVariableModuleGlobal->VariableGlobal.AuthFormat),
      ...
      //
      AuthStartPtr =
        (AUTHENTICATED_VARIABLE_HEADER *)(NextPtr + AuthStartPtr->DataSize + GET_PAD_SIZE (AuthStartPtr->DataSize));
    }
    ...
    if (GetVariableStoreStatus (VariableStoreHeader) == EfiValid) {
      if (!NeedConvertNormalToAuth) {
        mVariableModuleGlobal->VariableGlobal.HobVariableBase =
          (EFI_PHYSICAL_ADDRESS)(UINTN)AllocateRuntimeCopyPool ((UINTN)VariableStoreLength,
                                         (VOID *)VariableStoreHeader);
      } else {
        mVariableModuleGlobal->VariableGlobal.HobVariableBase =
          (EFI_PHYSICAL_ADDRESS)(UINTN)ConvertNormalVarStorageToAuthVarStorage ((VOID *)VariableStoreHeader);
      }
  ...
  )
{
  ...
  mVariableModuleGlobal->VolatileLastVariableOffset = (UINTN)GetStartPointer (VolatileVariableStore) -
