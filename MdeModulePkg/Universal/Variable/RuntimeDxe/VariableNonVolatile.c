  ...
  )
{
  ...
  *VariableStoreBase = (EFI_PHYSICAL_ADDRESS)(UINTN)VariableStore;
  ...
  //
  ASSERT (
    HwErrStorageSize < (VariableStoreLength -
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    ...
    if (GuidHob != NULL) {
      ...
      if (FtwLastWriteData->TargetAddress == NvStorageBase) {
        ...
        CopyMem (
          NvStorageData,
          (UINT8 *)(UINTN)(FtwLastWriteData->SpareAddress),
          NvStorageSize
          );
  ...
  *VariableStoreBase = (EFI_PHYSICAL_ADDRESS)(UINTN)VariableStore;
  ...
  //
  ASSERT (
    MaxUserNvVariableSpaceSize < (VariableStoreLength -
                                  sizeof (VARIABLE_STORE_HEADER) -
                                  HwErrStorageSize)
    );
  //
  ...
  //
  ASSERT (
    BoottimeReservedNvVariableSpaceSize < (VariableStoreLength -
                                           sizeof (VARIABLE_STORE_HEADER) -
                                           HwErrStorageSize)
    );
  ...
  //
  ASSERT (
    GetNonVolatileMaxVariableSize () < (VariableStoreLength -
                                        sizeof (VARIABLE_STORE_HEADER))
    );
