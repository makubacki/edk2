  ...
  )
{
  ...
  if (ConditionalExprCount > 0) {
    ...
    Statement->Expression = (FORM_EXPRESSION_LIST *)AllocatePool (
                                                      (UINTN)(sizeof (FORM_EXPRESSION_LIST) +
                                                              ((ConditionalExprCount -1) * sizeof (FORM_EXPRESSION *)))
                                                      );
  ...
  while (OpCodeOffset < FormSet->IfrBinaryLength) {
    ...
    if (IsExpressionOpCode (Operand)) {
      ...
      switch (Operand) {
        ...
        case EFI_IFR_EQ_ID_VAL_LIST_OP:
          ...
          ExpressionOpCode->ValueList =
            AllocateCopyPool (
              ExpressionOpCode->ListLength * sizeof (UINT16),
    ...
    switch (Operand) {
      ...
      case EFI_IFR_STRING_OP:
        ...
        CurrentStatement->HiiValue.Value.string =
      ...
      case EFI_IFR_DEFAULT_OP:
        ...
        if (CurrentDefault->Value.Type == EFI_IFR_TYPE_BUFFER) {
          ...
          CurrentDefault->Value.Buffer    =
            AllocateCopyPool (CurrentDefault->Value.BufferLen, &((EFI_IFR_DEFAULT *)OpCodeData)->Value);
          ASSERT (CurrentDefault->Value.Buffer != NULL);
        } else {
          CopyMem (
            &CurrentDefault->Value.Value,
            &((EFI_IFR_DEFAULT *)OpCodeData)->Value,
            OpCodeLength - OFFSET_OF (EFI_IFR_DEFAULT, Value)
            );
      ...
      case EFI_IFR_ONE_OF_OPTION_OP:
        ...
        if (ParentStatement->Operand == EFI_IFR_ORDERED_LIST_OP &&
            ((((EFI_IFR_ONE_OF_OPTION *)OpCodeData)->Flags & (EFI_IFR_OPTION_DEFAULT | EFI_IFR_OPTION_DEFAULT_MFG)) !=
             0)) {
          //
          ...
          CurrentDefault->Value.Buffer    =
            AllocateCopyPool (CurrentDefault->Value.BufferLen, &((EFI_IFR_ONE_OF_OPTION *)OpCodeData)->Value);
          ASSERT (CurrentDefault->Value.Buffer != NULL);
        ...
        CopyMem (
          ...
          OpCodeLength - OFFSET_OF (EFI_IFR_ONE_OF_OPTION, Value)
          );
