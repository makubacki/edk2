        ...
        if (OpCode->VarStorage != NULL) {
          switch (OpCode->VarStorage->Type) {
            ...
            case EFI_HII_VARSTORE_EFI_VARIABLE_BUFFER:
              ...
              CopyMem (
                &Value->Value,
                OpCode->VarStorage->EditBuffer + OpCode->VarStoreInfo.VarOffset,
                OpCode->ValueWidth
                );
              break;
      ...
      case EFI_IFR_QUESTION_REF3_OP:
        ...
        if (OpCode->DevicePath != 0) {
          ...
          if (StrPtr != NULL && mPathFromText != NULL) {
            ...
            if (DevicePath != NULL &&
                GetQuestionValueFromForm (DevicePath, NULL, &OpCode->Guid, Value->Value.u16, &QuestionVal)) {
              Value = &QuestionVal;
            }
            ...
            case EFI_HII_VARSTORE_NAME_VALUE:
              if (OpCode->ValueType != EFI_IFR_TYPE_STRING) {
                ...
                for (Index = 0; Index < OpCode->ValueWidth; Index++, TempBuffer--) {
                  ...
                  StrPtr +=
                    StrnLenS (StrPtr,
                      OpCode->ValueWidth * 2 + 1 - ((UINTN)StrPtr - (UINTN)NameValue) / sizeof (CHAR16));
                }
