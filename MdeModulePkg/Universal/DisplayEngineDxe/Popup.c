  ...
  )
{
  ...
  switch (PopupType) {
    ...
    case EfiHiiPopupTypeOkCancel:
      ...
      Status    = AddOneSelectableOption (PopupType, EfiHiiPopupSelectionOk, gOkOption, OptionCol, OptionRow);
      OptionCol = EndCol - (ColDimension - USER_SELECTABLE_OPTION_OK_CAL_WIDTH) / 3 -
    ...
    case EfiHiiPopupTypeYesNo:
      ...
      OptionCol = EndCol - (ColDimension - USER_SELECTABLE_OPTION_YES_NO_WIDTH) / 3 - (GetStringWidth (gNoOption)- 2) /
                  2 + 1;
    ...
    case EfiHiiPopupTypeYesNoCancel:
      ...
      OptionCol = EndCol - (ColDimension - USER_SELECTABLE_OPTION_YES_NO_CAL_WIDTH) / 4 - (GetStringWidth (
                                                                                             gCancelOption
                                                                                             ) - 2) / 2 + 1;
      Status    = AddOneSelectableOption (PopupType, EfiHiiPopupSelectionCancel, gCancelOption, OptionCol, OptionRow);
      break;
  ...
  )
{
  ...
  *OutputStrWidth = 0;
  ...
  //
  for (StrOffset = 0;
       InputString[*Index + StrOffset] != CHAR_CARRIAGE_RETURN && InputString[*Index + StrOffset] != CHAR_LINEFEED &&
       InputString[*Index + StrOffset] != CHAR_NULL;
       StrOffset++
       ) {
  }
  ...
  )
{
  ...
  for (Index = DisplayRow, StringIndex = 0;
       ParseMessageString (
         gMessageString,
         &OutputString,
         &OutputStrWidth,
         &StringIndex
         ) != 0 && DrawMesStrRowNum < gMesStrLineNum;
       ) {
