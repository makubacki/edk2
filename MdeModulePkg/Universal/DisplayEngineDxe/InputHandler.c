    ...
    switch (Key.UnicodeChar) {
          ...
          case SCAN_DELETE:
            for (Index = CurrentCursor; StringPtr[Index] != CHAR_NULL; Index++) {
              ...
              PrintCharAt (
                Start + Index + 1,
                Top + 3,
                IsPassword && StringPtr[Index] != CHAR_NULL ? L'*' : StringPtr[Index]
                );
            }
      ...
      default:
        ...
        if ((StringPtr[0] == CHAR_NULL) && (Key.UnicodeChar != CHAR_BACKSPACE)) {
          ...
          CurrentCursor++;
        } else if ((GetStringWidth (StringPtr) < ((Maximum + 1) * sizeof (CHAR16))) &&
  ...
  )
{
  ...
  if (Question->OpCode->OpCode == EFI_IFR_DATE_OP) {
    ...
    GetValueFromNum (
      Question->OpCode,
      (NumericOp->Flags & EFI_IFR_DISPLAY) == 0,
      QuestionValue,
      &EditValue,
      &Minimum,
      &Maximum,
  ...
  )
{
  ...
  for (Index = 0; Index < PopUpMenuLines; Index++) {
    ...
    if (!OrderedList &&
        (CompareHiiValue (&Question->CurrentValue, &HiiValue, &Result, NULL) == EFI_SUCCESS) && (Result == 0)) {
      //
