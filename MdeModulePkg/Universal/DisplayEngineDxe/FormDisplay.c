  ...
  )
{
  ...
  if ((gFormData->Attribute & HII_DISPLAY_MODAL) != 0) {
    ...
    return (UINT16)(gStatementDimensions.RightColumn - gStatementDimensions.LeftColumn - 2 *
                    (gModalSkipColumn + LEFT_SKIPPED_COLUMNS));
  }
    ...
    if (gHighligthMenuInfo.HLTQuestionId != 0) {
      ...
      if (CompareMem (
        ...
        if (gHighligthMenuInfo.HLTIndex == 0 ||
            gHighligthMenuInfo.HLTIndex == GetIndexInfoForOpcode (HighLightedStatement->OpCode)) {
          return TRUE;
        } else {
          return FALSE;
        }
  ...
  )
{
  if (gHighligthMenuInfo.HLTQuestionId != 0) {
    ...
    if(CompareMem (
      ...
      if (gHighligthMenuInfo.HLTIndex == 0 ||
          gHighligthMenuInfo.HLTIndex == GetIndexInfoForOpcode (MenuOption->ThisTag->OpCode)) {
        return (BOOLEAN)(MenuOption->Sequence == gHighligthMenuInfo.HLTSequence);
      } else {
        return FALSE;
      }
  ...
  )
{
  ...
  if(CompareMem (
    ...
    if (gHighligthMenuInfo.TOSIndex == 0 ||
        gHighligthMenuInfo.TOSIndex == GetIndexInfoForOpcode (MenuOption->ThisTag->OpCode)) {
      return TRUE;
    } else {
      return FALSE;
    }
      ...
      case CfUpdateHelpString:
        ...
        if (Repaint || NewLine) {
          if (IsListEmpty (&gMenuOption)) {
            ...
            HelpInfo =
              ((EFI_IFR_STATEMENT_HEADER *)((CHAR8 *)MenuOption->ThisTag->OpCode + sizeof (EFI_IFR_OP_HEADER)))->Help;
            Statement = MenuOption->ThisTag;
            StatementValue = &Statement->CurrentValue;
            if (HelpInfo == 0 || !IsSelectable (MenuOption)) {
              if ((Statement->OpCode->OpCode == EFI_IFR_DATE_OP && StatementValue->Value.date.Month == 0xff) ||
                  (Statement->OpCode->OpCode == EFI_IFR_TIME_OP && StatementValue->Value.time.Hour == 0xff)) {
      ...
      case CfReadKey:
        ...
        switch (Key.UnicodeChar) {
          ...
          case ' ':
            ...
            ASSERT (MenuOption != NULL);
            if (MenuOption->ThisTag->OpCode->OpCode == EFI_IFR_CHECKBOX_OP && !MenuOption->GrayOut &&
                !MenuOption->ReadOnly) {
              ScreenOperation = UiSelect;
            }
      ...
      case CfUiDown:
        ...
        if (MenuOption->Row + MenuOption->Skip - Temp2 > BottomRow + 1) {
          ...
          Difference = MoveToNextStatement (
                         FALSE,
                         &NewPos,
                         BottomRow + 1 - (MenuOption->Row + MenuOption->Skip - Temp2),
                         FALSE
                         );
        }
  ...
  )
{
  ...
  gPromptBlockWidth =
    (CHAR16)(gStatementDimensions.RightColumn - gStatementDimensions.LeftColumn - 2 * (gOptionBlockWidth - 1) - 1);
