/** @file
  This PEIM implements Firmware Volume Block (FVB) Variable Storage Services
  and installs an instance of the VariableStorage PPI for FVB variable support.
...
// Module globals
//
static EDKII_VARIABLE_STORAGE_PPI mFvbVariableStoragePpi = {
  ...
  )
{
  ...
  if (!ImageInPermanentMemory) {
    ...
    //
    // Search for any instances of EDKII_VARIABLE_STORAGE_PPI installed
    ...
    //
    // This PEIM did not install an instance of EDKII_VARIABLE_STORAGE_PPI
...
/**
  ...
  @retval  EFI_NOT_FOUND          Variable not found or Variable store is corrupted or invalid
  ...
  )
{
  ...
  if (*DataSize >= VarDataSize) {
    ...
    if (Attributes != NULL && VariableHeader != NULL ) {
