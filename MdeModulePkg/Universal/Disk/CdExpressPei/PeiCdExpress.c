  ...
  )
{
  ...
  for (BlockIoPpiInstance = 0; BlockIoPpiInstance < PEI_CD_EXPRESS_MAX_BLOCK_IO_PPI; BlockIoPpiInstance++) {
    ...
    //
    for (IndexBlockDevice = 1;
         IndexBlockDevice <= NumberBlockDevices && PrivateData->CapsuleCount < PEI_CD_EXPRESS_MAX_CAPSULE_NUMBER;
         IndexBlockDevice++) {
      if (BlockIo2) {
  ...
  )
{
  ...
  for (Index = 0; Index < Size; Index++) {
    ...
    if (!CaseSensitive) {
      Dif =
        (UINT8)((Source1[Index] >
                 Source2[Index]) ? (Source1[Index] - Source2[Index]) : (Source2[Index] - Source1[Index]));
      if (Dif == ('a' - 'A')) {
        continue;
      }
    }
