  ...
  )
{
  ...
  mWorkingBlockHeader.WriteQueueSize = PcdGet32 (PcdFlashNvStorageFtwWorkingSize) -
                                       sizeof (EFI_FAULT_TOLERANT_WORKING_BLOCK_HEADER);
  ...
  )
{
  ...
  RemainingSpaceSize = FtwDevice->FtwWorkSpaceSize -
                       ((UINTN)FtwDevice->FtwLastWriteHeader - (UINTN)FtwDevice->FtwWorkSpace);
  ...
  //
  if (EFI_ERROR (Status) ||
      RemainingSpaceSize < sizeof (EFI_FAULT_TOLERANT_WRITE_HEADER) + sizeof (EFI_FAULT_TOLERANT_WRITE_RECORD)) {
    //
