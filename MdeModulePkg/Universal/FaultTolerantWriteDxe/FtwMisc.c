  ...
  )
{
  ...
  //
  WorkingBlockHeader =
    (EFI_FAULT_TOLERANT_WORKING_BLOCK_HEADER *)(Buffer + (UINTN)FtwDevice->FtwWorkSpaceLbaInSpare *
                                                FtwDevice->SpareBlockSize + FtwDevice->FtwWorkSpaceBaseInSpare);
  ...
  )
{
  ...
  *FtwWriteRecord = NULL;
  ...
  if (Index == FtwWriteHeader->NumberOfWrites) {
    *FtwWriteRecord =
      (EFI_FAULT_TOLERANT_WRITE_RECORD *)((UINTN)FtwRecord - FTW_RECORD_SIZE (FtwWriteHeader->PrivateDataSize));
    return EFI_SUCCESS;
  }
  ...
  )
{
  ...
  FtwDevice->FtwWorkBlockLba = FtwDevice->FtwWorkSpaceLba + FtwDevice->NumberOfWorkSpaceBlock -
                               FtwDevice->NumberOfWorkBlock;
  DEBUG ((
    EFI_D_INFO,
  ...
  FtwDevice->FtwWorkSpaceLbaInSpare =
    (EFI_LBA)(((UINTN)WorkSpaceLbaOffset * FtwDevice->WorkBlockSize + FtwDevice->FtwWorkSpaceBase) /
              FtwDevice->SpareBlockSize);
  ...
  if ((FtwDevice->FtwLastWriteHeader->Complete != FTW_VALID_STATE) &&
    ...
    if (FtwDevice->FtwLastWriteRecord->BootBlockUpdate == FTW_VALID_STATE) {
      ...
      FvbHandle =
        GetFvbByAddress (
          (EFI_PHYSICAL_ADDRESS)(UINTN)((INT64)FtwDevice->SpareAreaAddress +
                                        FtwDevice->FtwLastWriteRecord->RelativeOffset),
          &Fvb
          );
      if (FvbHandle != NULL) {
