  ...
  )
{
  ...
  DEBUG (
        (EFI_D_ERROR, "SdPeimTuningClock: Send tuning block fails at %d times with HostCtrl2 %02x\n", Retry,
         HostCtrl2)
        );
  //
  ...
  )
{
  ...
  do {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG (
            (EFI_D_ERROR, "SdPeimIdentification: SdPeimSendOpCond fails with %r Ocr %x, S18r %x, Xpc %x\n", Status,
             Ocr, S18r, Xpc)
            );
      return EFI_DEVICE_ERROR;
    }
  ...
  if ((Capability.Sdr50 != 0 ||
    ...
    if (EFI_ERROR (Status)) {
      ...
      if (((PresentState >> 20) & 0xF) != 0xF) {
        DEBUG (
              (EFI_D_ERROR, "SdPeimIdentification: SwitchVoltage fails with PresentState = 0x%x, It should be 0xF\n",
               PresentState)
              );
        Status = EFI_DEVICE_ERROR;
        goto Error;
