  ...
  )
{
  ...
  *Consumed = 0;
  ...
  DEBUG (
         (EFI_D_INFO, "UsbParseInterfaceDesc: interface %d(setting %d) has %d endpoints\n",
  ...
  )
{
  ...
  DEBUG (
         (EFI_D_INFO, "UsbParseConfigDesc: config %d has %d interfaces\n",
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG (
           (EFI_D_ERROR, "UsbGetOneConfig: failed to get descript length(%d) %r\n",
