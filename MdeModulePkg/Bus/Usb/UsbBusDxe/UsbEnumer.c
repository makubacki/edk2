  ...
  )
{
  ...
  ASSERT (Setting != NULL);
  DEBUG (
         (EFI_D_INFO, "UsbSelectSetting: setting %d selected for interface %d\n",
  ...
  )
{
  ...
  DEBUG (
         (EFI_D_INFO, "UsbSelectConfig: config %d selected for device %d\n",
  ...
  )
{
  ...
  DEBUG (
         (EFI_D_INFO, "UsbEnumerateNewDev: device uses translator (%d, %d)\n",
  ...
  )
{
  ...
  DEBUG (
         (EFI_D_INFO, "UsbEnumeratePort: port %d state - %02x, change - %02x on %p\n",
  ...
  )
{
  ...
  for (Index = 0; Index < HubIf->NumOfPort; Index++) {
    ...
    if ((Child != NULL) && (Child->DisconnectFail == TRUE)) {
      DEBUG (
            (EFI_D_INFO, "UsbEnumeratePort: The device disconnect fails at port %d from hub %p, try again\n", Index,
             HubIf)
            );
      UsbRemoveDevice (Child);
    }
  ...
  )
{
  ...
  for (Index = 0; Index < RootHub->NumOfPort; Index++) {
    ...
    if ((Child != NULL) && (Child->DisconnectFail == TRUE)) {
      DEBUG (
            (EFI_D_INFO, "UsbEnumeratePort: The device disconnect fails at port %d from root hub %p, try again\n",
             Index, RootHub)
            );
      UsbRemoveDevice (Child);
    }
