...
// Use a shor form Usb class Device Path, which could match any usb device, in WantedUsbIoDPList to indicate all Usb
// devices
  ...
  for (Index = 0; Index < UsbIoHandleCount; Index++) {
    ...
    if (UsbBusIsWantedUsbIO (Bus, UsbIf)) {
      if (!UsbIf->IsManaged) {
        ...
        //
        DEBUG (
              (EFI_D_INFO, "UsbBusRecursivelyConnectWantedUsbIo: TPL before connect is %d\n",
               (UINT32) UsbGetCurrentTpl ())
              );
        Status = gBS->ConnectController (UsbIf->Handle, NULL, NULL, TRUE);
        UsbIf->IsManaged = (BOOLEAN) !EFI_ERROR (Status);
        DEBUG (
              (EFI_D_INFO, "UsbBusRecursivelyConnectWantedUsbIo: TPL after connect is %d\n",
               (UINT32) UsbGetCurrentTpl ())
              );
      }
    }
