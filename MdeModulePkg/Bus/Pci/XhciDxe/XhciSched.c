  ...
  )
{
  ...
  if (Urb->Finished && Urb->Result == EFI_USB_NOERROR) {
    ...
    DEBUG (
          (DEBUG_INFO, "XhcDequeueTrbFromEndpoint: Pending URB is finished: Length Actual/Expect = %d/%d!\n",
    ...
    if (EFI_ERROR (Status)) {
      DEBUG (
            (DEBUG_ERROR, "XhcDequeueTrbFromEndpoint: Set Transfer Ring Dequeue Pointer Failed, Status = %r\n",
             Status)
            );
      goto Done;
    }
  ...
  )
{
  ...
  if (Xhc->ScratchBuf != NULL) {
    ...
    UsbHcFreeAlignedPages (
                          Xhc->PciIo,
                          Xhc->ScratchBuf,
                          EFI_SIZE_TO_PAGES (Xhc->MaxScratchpadBufs * sizeof (UINT64)),
                          Xhc->ScratchMap
                          );
  ...
  )
{
  ...
  for (Index = 0; Index < Urb->TrbNum; Index++) {
    ...
    if (CheckedTrb->Type == TRB_TYPE_LINK) {
      ...
      CheckedTrb =
        (TRB_TEMPLATE *) (UINTN) UsbHcGetHostAddrForPciAddr (
                                                                        Xhc->MemPool,
                                                                        (VOID *) (UINTN) PhyAddr,
                                                                        sizeof (TRB_TEMPLATE)
                                                                        );
  ...
  )
{
  ...
  for (Index = 0; Index < Xhc->EventRing.TrbNumber; Index++) {
    ...
    TRBPtr  =
      (TRB_TEMPLATE *) (UINTN) UsbHcGetHostAddrForPciAddr (Xhc->MemPool, (VOID *) (UINTN) PhyAddr,
                                                           sizeof (TRB_TEMPLATE));
  ...
  )
{
  ...
  if ((PortState->PortChangeStatus &
       (USB_PORT_STAT_C_CONNECTION | USB_PORT_STAT_C_ENABLE | USB_PORT_STAT_C_OVERCURRENT | USB_PORT_STAT_C_RESET)) ==
      0) {
    return EFI_SUCCESS;
  }
  ...
  if (ParentRouteChart.Dword == 0) {
    ...
    if(Port < 14) {
      RouteChart.Route.RouteString = ParentRouteChart.Route.RouteString |
                                     (Port << (4 * (ParentRouteChart.Route.TierNum - 1)));
    } else {
      RouteChart.Route.RouteString = ParentRouteChart.Route.RouteString |
  ...
  )
{
  ...
  *NewEvtTrb = EvtRing->EventRingDequeue;
  ...
  //
  if ((UINTN) EvtRing->EventRingDequeue >=
      ((UINTN) EvtRing->EventRingSeg0 + sizeof (TRB_TEMPLATE) * EvtRing->TrbNumber)) {
  ...
  )
{
  ...
  ZeroMem (&CmdTrbAddr, sizeof (CmdTrbAddr));
  PhyAddr =
    UsbHcGetPciAddrForHostAddr (Xhc->MemPool, Xhc->UsbDevContext[SlotId].InputContext, sizeof (INPUT_CONTEXT_64));
  CmdTrbAddr.PtrLo    = XHC_LOW_32BIT (PhyAddr);
  CmdTrbAddr.PtrHi    = XHC_HIGH_32BIT (PhyAddr);
  CmdTrbAddr.CycleBit = 1;
  CmdTrbAddr.Type     = TRB_TYPE_ADDRESS_DEV;
  ...
  )
{
  ...
  for (EpIndex = 0; EpIndex < NumEp; EpIndex++) {
    ...
    if (DeviceSpeed == EFI_USB_SPEED_SUPER) {
      //
      // 6.2.3.4, shall be set to the value defined in the bMaxBurst field of the SuperSpeed Endpoint Companion
      // Descriptor.
      //
    ...
    switch (EpDesc->Attributes & USB_ENDPOINT_TYPE_MASK) {
      case USB_ENDPOINT_BULK:
        ...
        if (Xhc->UsbDevContext[SlotId].EndpointTransferRing[Dci-1] == NULL) {
          ...
          DEBUG (
                 (DEBUG_INFO, "Endpoint[%x]: Created BULK ring [%p~%p)\n",
      ...
      case USB_ENDPOINT_INTERRUPT:
        ...
        if (Xhc->UsbDevContext[SlotId].EndpointTransferRing[Dci-1] == NULL) {
          ...
          DEBUG (
                 (DEBUG_INFO, "Endpoint[%x]: Created INT ring [%p~%p)\n",
                  EpDesc->EndpointAddress,
                  EndpointTransferRing->RingSeg0,
                  (UINTN) EndpointTransferRing->RingSeg0 + TR_RING_TRB_NUMBER * sizeof (TRB_TEMPLATE)
                 )
    ...
    PhyAddr |=
      (EFI_PHYSICAL_ADDRESS) ((TRANSFER_RING *) (UINTN) Xhc->UsbDevContext[SlotId].EndpointTransferRing[Dci-
                                                                                                        1])->RingPCS;
    ...
    switch (EpDesc->Attributes & USB_ENDPOINT_TYPE_MASK) {
      case USB_ENDPOINT_BULK:
        ...
        if (Xhc->UsbDevContext[SlotId].EndpointTransferRing[Dci-1] == NULL) {
          ...
          DEBUG (
                 (DEBUG_INFO, "Endpoint64[%x]: Created BULK ring [%p~%p)\n",
                  EpDesc->EndpointAddress,
                  EndpointTransferRing->RingSeg0,
                  (UINTN) EndpointTransferRing->RingSeg0 + TR_RING_TRB_NUMBER * sizeof (TRB_TEMPLATE)
                 )
      ...
      case USB_ENDPOINT_INTERRUPT:
        ...
        if (Xhc->UsbDevContext[SlotId].EndpointTransferRing[Dci-1] == NULL) {
          ...
          DEBUG (
                 (DEBUG_INFO, "Endpoint64[%x]: Created INT ring [%p~%p)\n",
                  EpDesc->EndpointAddress,
                  EndpointTransferRing->RingSeg0,
                  (UINTN) EndpointTransferRing->RingSeg0 + TR_RING_TRB_NUMBER * sizeof (TRB_TEMPLATE)
                 )
