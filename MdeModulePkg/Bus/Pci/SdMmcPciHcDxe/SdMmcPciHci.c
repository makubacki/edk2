  ...
  )
{
  ...
  DEBUG (
        (DEBUG_INFO, "   Timeout Clk Freq  %d%a\n", Capability->TimeoutFreq,
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG (
           (DEBUG_INFO, "SdMmcHcReset: SdMmcHcEnableInterrupt done with %r\n",
            Status)
           );
    return Status;
  }
  ...
  )
{
  ...
  if (Packet->SdMmcStatusBlk != NULL) {
    DEBUG (
           (DebugLevel, "Response 0: %X, 1: %X, 2: %X, 3: %X\n",
