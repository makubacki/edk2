  ...
  )
{
  ...
  DEBUG (
        (DEBUG_ERROR, "SdCardTuningClock: Send tuning block fails at %d times with HostCtrl2 %02x\n", Retry,
         HostCtrl2)
        );
  //
  ...
  )
{
  ...
  if ((BusMode.BusTiming == SdMmcUhsSdr104) ||
      ((BusMode.BusTiming == SdMmcUhsSdr50) && (Capability->TuningSDR50 != 0))) {
  ...
  )
{
  ...
  do {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG (
            (DEBUG_ERROR, "SdCardIdentification: SdCardSendOpCond fails with %r Ocr %x, S18r %x, Xpc %x\n", Status,
             Ocr, S18r, Xpc)
            );
      return EFI_DEVICE_ERROR;
    }
  ...
  if ((Private->Capability[Slot].Sdr50 != 0 ||
    ...
    if (EFI_ERROR (Status)) {
      ...
      if (((PresentState >> 20) & 0xF) != 0xF) {
        DEBUG (
              (DEBUG_ERROR, "SdCardIdentification: SwitchVoltage fails with PresentState = 0x%x, It should be 0xF\n",
               PresentState)
              );
        Status = EFI_DEVICE_ERROR;
        goto Error;
