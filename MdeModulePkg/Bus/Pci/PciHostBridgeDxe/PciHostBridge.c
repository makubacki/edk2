  ...
  )
{
  ...
  DEBUG (
         (EFI_D_ERROR, "%a: %a: desc [%Lx, %Lx) type %u conflicts with "
                       "aperture [%Lx, %Lx)\n", gEfiCallerBaseName, __FUNCTION__,
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG (
           (EFI_D_ERROR, "%a: %a: GetIoSpaceMap(): %r\n",
  ...
  )
{
  ...
  DEBUG (
         (EFI_D_ERROR, "%a: %a: desc [%Lx, %Lx) type %u cap %Lx conflicts "
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG (
           (EFI_D_ERROR, "%a: %a: GetMemorySpaceMap(): %r\n",
            gEfiCallerBaseName, __FUNCTION__, Status)
           );
    return Status;
  }
        ...
        for (Index1 = TypeIo; Index1 < TypeBus; Index1++) {
          if (RootBridge->ResAllocNode[Index1].Status == ResNone) {
            ...
            if ((Translation & Alignment) != 0) {
              DEBUG (
                     (DEBUG_ERROR, "[%a:%d] Translation %lx is not aligned to %lx!\n",
            ...
            DEBUG (
                   (DEBUG_INFO, "  %s: Base/Length/Alignment = %lx/%lx/%lx - ",
  ...
  for (Link = GetFirstNode (&HostBridge->RootBridges)
    ...
    if (RootBridgeHandle == RootBridge->Handle) {
      ...
      for (Descriptor = (EFI_ACPI_ADDRESS_SPACE_DESCRIPTOR *) Configuration;
        ...
        DEBUG (
               (EFI_D_INFO, " %s: Granularity/SpecificFlag = %ld / %02x%s\n",
                mAcpiAddressSpaceTypeStr[Descriptor->ResType], Descriptor->AddrSpaceGranularity,
  ...
  )
{
  ...
  for (Link = GetFirstNode (&HostBridge->RootBridges)
    ...
    if (RootBridgeHandle == RootBridge->Handle) {
      ...
      Buffer =
        AllocateZeroPool (Number * sizeof (EFI_ACPI_ADDRESS_SPACE_DESCRIPTOR) + sizeof (EFI_ACPI_END_TAG_DESCRIPTOR));
      if (Buffer == NULL) {
        return EFI_OUT_OF_RESOURCES;
      }
