  ...
  )
{
  ...
  for (Index = 0; Index < Xhc->EventRing.TrbNumber; Index++) {
    ...
    TRBPtr  =
      (TRB_TEMPLATE *)(UINTN)UsbHcGetHostAddrForPciAddr (Xhc->MemPool, (VOID *)(UINTN)PhyAddr, sizeof (TRB_TEMPLATE));
  ...
  )
{
  ...
  for (Index = 0; Index < ConfigDesc->NumInterfaces; Index++) {
    ...
    for (EpIndex = 0; EpIndex < NumEp; EpIndex++) {
      ...
      PhyAddr |=
        (EFI_PHYSICAL_ADDRESS)((TRANSFER_RING *)(UINTN)Xhc->UsbDevContext[SlotId].EndpointTransferRing[Dci-1])->RingPCS;
