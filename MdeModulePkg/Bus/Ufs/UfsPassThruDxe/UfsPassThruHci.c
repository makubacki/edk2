  ...
  )
{
  ...
  TotalLen = ROUNDUP8 (sizeof (UTP_COMMAND_UPIU)) + ROUNDUP8 (sizeof (UTP_RESPONSE_UPIU)) + PrdtNumber *
             sizeof (UTP_TR_PRD);
  ...
  )
{
  ...
  if (Opcode == UtpQueryFuncOpcodeWrDesc) {
    ...
    Trd->RuO = (UINT16) DivU64x32 (
                                  (UINT64) ROUNDUP8 (sizeof (UTP_QUERY_REQ_UPIU)) + ROUNDUP8 (DataSize),
                                  sizeof (UINT32)
                                  );
  } else {
    Trd->RuL =
      (UINT16) DivU64x32 ((UINT64) ROUNDUP8 (sizeof (UTP_QUERY_RESP_UPIU)) + ROUNDUP8 (DataSize), sizeof (UINT32));
  ...
  )
{
  ...
  //
  PrdtBase =
    (UTP_TR_PRD *) ((UINT8 *) TransReq->CmdDescHost + ROUNDUP8 (sizeof (UTP_COMMAND_UPIU)) +
                    ROUNDUP8 (sizeof (UTP_RESPONSE_UPIU)));
  ...
  if (!IsListEmpty (&Private->Queue)) {
    BASE_LIST_FOR_EACH_SAFE (Entry, NextEntry, &Private->Queue) {
      ...
      if (EFI_ERROR (Status)) {
        ...
        DEBUG (
              (DEBUG_VERBOSE, "ProcessAsyncTaskList(): Signal Event %p UfsMmioRead32() Error.\n",
      ...
      if ((Value & (BIT0 << TransReq->Slot)) != 0) {
        ...
        if (TransReq->Trd->Ocs == 0) {
          ...
          DEBUG (
                (DEBUG_VERBOSE, "ProcessAsyncTaskList(): Signal Event %p Target Device Error.\n",
                 TransReq->CallerEvent)
                );
          SignalCallerEvent (Private, TransReq);
          continue;
