  ...
  )
{
  ...
  CommGetData =
    (SMI_HANDLER_PROFILE_PARAMETER_GET_DATA_BY_OFFSET *)&CommBuffer[OFFSET_OF (EFI_SMM_COMMUNICATE_HEADER, Data)];
  ...
  )
{
  ...
  if (CompareGuid (HandlerType, &gEfiSmmSwDispatch2ProtocolGuid)) {
    ...
    Print (
      ...
      PowerButtonPhaseToString (((EFI_SMM_POWER_BUTTON_REGISTER_CONTEXT *)Context)->Phase)
      );
  } else if (CompareGuid (HandlerType, &gEfiSmmStandbyButtonDispatch2ProtocolGuid)) {
    Print (
      L" StandbyButtonPhase=\"%a\"",
      StandbyButtonPhaseToString (((EFI_SMM_STANDBY_BUTTON_REGISTER_CONTEXT *)Context)->Phase)
      );
