  ...
  )
{
  ...
  for (Index = FmpCapsuleHeader->EmbeddedDriverCount; Index < Count; Index++) {
    ...
    FmpImageHeader =
      (EFI_FIRMWARE_MANAGEMENT_CAPSULE_IMAGE_HEADER *)((UINT8 *)FmpCapsuleHeader + ItemOffsetList[Index]);
  ...
  while (TRUE) {
    ...
    if (CompareGuid (&CapsuleResult->CapsuleGuid, &gEfiFmpCapsuleGuid)) {
      if (CapsuleResult->VariableTotalSize >=
          sizeof (EFI_CAPSULE_RESULT_VARIABLE_HEADER) + sizeof (EFI_CAPSULE_RESULT_VARIABLE_FMP) + sizeof (CHAR16) *
          2) {
  ...
  if (EFI_ERROR (Status) || BootNext == NULL) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      if(!EFI_ERROR (Status)) {
        ...
        Print (
          L"    %s %s\n",
          ShellProtocol->GetMapFromDevicePath (&DevicePath),
