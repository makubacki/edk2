...
#define IS_SMRAM_PROFILE_ENABLED        \
                                                                                          ((PcdGet8 (PcdMemoryProfilePropertyMask) & BIT1) != 0)
#define IS_UEFI_MEMORY_PROFILE_ENABLED  \
...
GLOBAL_REMOVE_IF_UNREFERENCED LIST_ENTRY
  mImageQueue = INITIALIZE_LIST_HEAD_VARIABLE (mImageQueue);
  ...
  )
{
                                                          ...
                                                          ImageSize))))
  {
    ...
    DriverInfo->ImageBase = ImageBase + EntryPoint -
                            (PHYSICAL_ADDRESS)(UINTN)EntryPointInImage;
  }
  ...
  )
{
  ...
  do {
    ...
    //
    DevicePathInstance =
      (EFI_DEVICE_PATH_PROTOCOL *)((UINTN)DevicePathInstance + DevicePathSize +
  ...
  )
{
  ...
  mSmramProfileDriverPath     = AllocateCopyPool (
                                  mSmramProfileDriverPathSize,
  ...
  )
{
  ...
  if ((DriverEntry->ImageEntryPoint < ImageAddress) ||
      (DriverEntry->ImageEntryPoint >= (ImageAddress + EFI_PAGES_TO_SIZE (
                                                         DriverEntry->
                                                           NumberOfPage
                                                         ))))
  {
    ...
    ASSERT_EFI_ERROR (Status);
    ImageAddress = ImageAddress + (UINTN)DriverEntry->ImageEntryPoint -
  ...
  if (DriverInfoData == NULL) {
    DriverInfoData = GetMemoryProfileDriverInfoFromAddress (
                       ContextData,
                       ImageAddress
                       );
  }
  ...
  )
{
  ...
  if (ActionString != NULL) {
    ...
    ActionStringOccupiedSize = GET_OCCUPIED_SIZE (
                                 ActionStringSize,
                                 sizeof (UINT64)
                                 );
  }
    ...
    switch (BasicAction) {
      case MemoryProfileActionAllocatePages:
        if ((AllocInfo->Buffer <= (PHYSICAL_ADDRESS)(UINTN)Buffer) &&
            ((AllocInfo->Buffer + AllocInfo->Size) >=
  ...
  )
{
  ...
  do {
    ...
    if (BasicAction == MemoryProfileActionFreePages) {
      ...
      if (AllocInfo->Buffer + AllocInfo->Size !=
          ((PHYSICAL_ADDRESS)(UINTN)Buffer + Size))
      {
        SmmCoreUpdateProfileAllocate (
          AllocInfo->CallerAddress,
  ...
  )
{
  ...
  switch (BasicAction) {
    case MemoryProfileActionAllocatePages:
      Status = SmmCoreUpdateProfileAllocate (
                 CallerAddress,
                 Action,
                 MemoryType,
                 Size,
  ...
  )
{
  ...
  TotalSize += (sizeof (MEMORY_PROFILE_FREE_MEMORY) + Index *
  ...
  )
{
    ...
    if (*ProfileOffset < (Offset + DriverInfoData->DriverInfo.Header.Length)) {
      if (RemainingSize >= DriverInfoData->DriverInfo.Header.Length) {
        ...
        if (DriverInfo->PdbStringOffset != 0) {
          ...
          CopyMem (
            (VOID *)((UINTN)DriverInfo + DriverInfo->PdbStringOffset),
            DriverInfoData->PdbString,
            PdbSize
            );
        }
  ...
  if (*ProfileOffset < (Offset + sizeof (MEMORY_PROFILE_FREE_MEMORY))) {
    if (RemainingSize >= sizeof (MEMORY_PROFILE_FREE_MEMORY)) {
      ...
      ProfileBuffer  = (UINT8 *)ProfileBuffer +
                       sizeof (MEMORY_PROFILE_FREE_MEMORY);
    } else {
      goto Done;
    }
       ...
       Node = Node->BackLink)
  {
    if (*ProfileOffset < (Offset + sizeof (MEMORY_PROFILE_DESCRIPTOR))) {
      if (RemainingSize >= sizeof (MEMORY_PROFILE_DESCRIPTOR)) {
        ...
        MemoryProfileDescriptor->Header.Length =
          sizeof (MEMORY_PROFILE_DESCRIPTOR);
        MemoryProfileDescriptor->Header.Revision =
          MEMORY_PROFILE_DESCRIPTOR_REVISION;
        MemoryProfileDescriptor->Address =
          (PHYSICAL_ADDRESS)(UINTN)Pages;
        MemoryProfileDescriptor->Size = EFI_PAGES_TO_SIZE (
                                          Pages->NumberOfPages
                                          );
  ...
  )
{
  ...
  SmramProfileParameterGetInfo->ProfileSize =
    SmramProfileGetDataSize ();
  ...
  )
{
  ...
  if (!SmmIsBufferOutsideSmmValid (
         (UINTN)SmramProfileGetData.ProfileBuffer,
         (UINTN)ProfileSize
         ))
  {
    DEBUG ((
    ...
    SmramProfileParameterGetData->Header.ReturnStatus =
  ...
  if (SmramProfileGetData.ProfileSize < ProfileSize) {
    SmramProfileParameterGetData->ProfileSize         = ProfileSize;
    SmramProfileParameterGetData->Header.ReturnStatus =
      (UINT64)(INT64)(INTN)EFI_BUFFER_TOO_SMALL;
    goto Done;
  }
...
SmramProfileHandlerGetDataByOffset (
  IN SMRAM_PROFILE_PARAMETER_GET_PROFILE_DATA_BY_OFFSET  *
  ...
  )
{
  ...
  //
  if (!SmmIsBufferOutsideSmmValid (
         (UINTN)SmramProfileGetDataByOffset.ProfileBuffer,
         (UINTN)SmramProfileGetDataByOffset.ProfileSize
         ))
  {
    DEBUG ((
      DEBUG_ERROR,
    ...
    SmramProfileParameterGetDataByOffset->Header.ReturnStatus =
      (UINT64)(INT64)(INTN)EFI_ACCESS_DENIED;
    goto Done;
  }

  SmramProfileCopyData (
    (VOID *)(UINTN)SmramProfileGetDataByOffset.ProfileBuffer,
    &SmramProfileGetDataByOffset.ProfileSize,
    &SmramProfileGetDataByOffset.ProfileOffset
    );
  CopyMem (
    SmramProfileParameterGetDataByOffset,
    &SmramProfileGetDataByOffset,
  ...
  )
{
  ...
  if (mSmramReadyToLock && !SmmIsBufferOutsideSmmValid (
                              (UINTN)CommBuffer,
                              TempCommBufferSize
                              ))
  {
  ...
  SmramProfileParameterHeader =
    (SMRAM_PROFILE_PARAMETER_HEADER *)((UINTN)CommBuffer);

  SmramProfileParameterHeader->ReturnStatus = (UINT64)-1;

  if (GetSmramProfileContext () == NULL) {
    SmramProfileParameterHeader->ReturnStatus =
      (UINT64)(INT64)(INTN)EFI_UNSUPPORTED;
    return EFI_SUCCESS;
  }
  ...
  switch (SmramProfileParameterHeader->Command) {
    case SMRAM_PROFILE_COMMAND_GET_PROFILE_INFO:
      ...
      SmramProfileHandlerGetInfo (
        (SMRAM_PROFILE_PARAMETER_GET_PROFILE_INFO *)(UINTN)CommBuffer
        );
      break;
    case SMRAM_PROFILE_COMMAND_GET_PROFILE_DATA:
      ...
      SmramProfileHandlerGetData (
        (SMRAM_PROFILE_PARAMETER_GET_PROFILE_DATA *)(UINTN)CommBuffer
        );
      break;
    ...
    case SMRAM_PROFILE_COMMAND_GET_RECORDING_STATE:
      ...
      ParameterRecordingState =
        (SMRAM_PROFILE_PARAMETER_RECORDING_STATE *)(UINTN)CommBuffer;
      ParameterRecordingState->RecordingState =
        mSmramProfileRecordingEnable;
