    ...
    if (LoadedImagePrivate->Signature == EFI_SMM_DRIVER_ENTRY_SIGNATURE) {
      ...
      if ((EntryPoint != 0) &&
          ((EntryPoint < (UINTN) LoadedImage->ImageBase) ||
           (EntryPoint >= ((UINTN) LoadedImage->ImageBase + LoadedImage->ImageSize)))) {
        //
  ...
  )
{
  ...
  if (CompareGuid (HandlerType, &gEfiSmmSwDispatch2ProtocolGuid)) {
    ...
    DEBUG (
          (DEBUG_INFO, "  PeriodicTimerPeriod - %ld\n",
           ((EFI_SMM_PERIODIC_TIMER_REGISTER_CONTEXT *) Context)->Period)
          );
    DEBUG (
          (DEBUG_INFO, "  PeriodicTimerSmiTickInterval - %ld\n",
    ...
    Str =
      ConvertDevicePathToText (
                                   (EFI_DEVICE_PATH_PROTOCOL *) (((SMI_HANDLER_PROFILE_USB_REGISTER_CONTEXT *) Context)
                                                                 + 1),
                                   TRUE,
                                   TRUE
                                   );
  ...
  )
{
  ...
  for (Index = 0; Index < mImageStructCount; Index++) {
    Size += sizeof (SMM_CORE_IMAGE_DATABASE_STRUCTURE) +
            GET_OCCUPIED_SIZE (mImageStruct[Index].PdbStringSize, sizeof (UINT64));
  }
  ...
  )
{
  ...
  for (Index = 0; Index < mImageStructCount; Index++) {
    ...
    if (sizeof (SMM_CORE_IMAGE_DATABASE_STRUCTURE) +
        GET_OCCUPIED_SIZE (mImageStruct[Index].PdbStringSize, sizeof (UINT64)) > ExpectedSize - Size) {
      return 0;
    }
  ...
  *Count    = 0;
  ...
  for (ListEntry = ListEntry->ForwardLink;
    ...
    if (sizeof (SMM_CORE_SMI_HANDLER_STRUCTURE) +
        GET_OCCUPIED_SIZE (SmiHandler->ContextSize, sizeof (UINT64)) > MaxSize - Size) {
      *Count = 0;
      return 0;
    }

    SmiHandlerStruct->Length =
      (UINT32) (sizeof (SMM_CORE_SMI_HANDLER_STRUCTURE) + GET_OCCUPIED_SIZE (SmiHandler->ContextSize, sizeof (UINT64)));
  ...
  )
{
  ...
  SmmSmiDatabaseSize = GetSmmSmiDatabaseData (
                                            ...
                                            (UINT8 *) Data + SmmImageDatabaseSize + mSmmRootSmiDatabaseSize,
                                            mSmmSmiDatabaseSize
                                            );
  ...
  SmmHardwareSmiDatabaseSize = GetSmmSmiDatabaseData (
                                                    ...
                                                    (UINT8 *) Data + SmmImageDatabaseSize + SmmRootSmiDatabaseSize + SmmSmiDatabaseSize,
  ...
  for (Link = Head->ForwardLink; Link != Head; Link = Link->ForwardLink) {
    ...
    if (SmiHandler->Handler == Handler) {
      if ((SearchContext == NULL) ||
          ((SearchContextSize == SmiHandler->ContextSize) &&
