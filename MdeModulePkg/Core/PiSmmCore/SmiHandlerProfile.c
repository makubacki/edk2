...
GLOBAL_REMOVE_IF_UNREFERENCED LIST_ENTRY  mHardwareSmiEntryList =
  INITIALIZE_LIST_HEAD_VARIABLE (mHardwareSmiEntryList);

GLOBAL_REMOVE_IF_UNREFERENCED LIST_ENTRY  mRootSmiEntryList =
  INITIALIZE_LIST_HEAD_VARIABLE (mRootSmiEntryList);

GLOBAL_REMOVE_IF_UNREFERENCED LIST_ENTRY  *mSmmCoreRootSmiEntryList =
  &mRootSmiEntryList;
GLOBAL_REMOVE_IF_UNREFERENCED LIST_ENTRY  *mSmmCoreSmiEntryList =
  &mSmiEntryList;
GLOBAL_REMOVE_IF_UNREFERENCED LIST_ENTRY  *mSmmCoreHardwareSmiEntryList =
  ...
  )
{
  ...
  {
    FileName =
      &((MEDIA_FW_VOL_FILEPATH_DEVICE_PATH *)LoadedImage->FilePath)->FvFileName;
  }
  ...
  )
{
  ...
  for (Index = 0; Index < mImageStructCount; Index++) {
    if ((Address >= mImageStruct[Index].ImageBase) &&
        (Address < mImageStruct[Index].ImageBase +
         mImageStruct[Index].ImageSize))
    {
  ...
  )
{
  ...
  Size = 0;
  for (Index = 0; Index < mImageStructCount; Index++) {
    Size += sizeof (SMM_CORE_IMAGE_DATABASE_STRUCTURE) + GET_OCCUPIED_SIZE (
                                                           mImageStruct[Index].
                                                             PdbStringSize,
                                                           sizeof (UINT64)
                                                           );
  ...
  )
{
  ...
  {
    SmiHandler = CR (ListEntry, SMI_HANDLER, Link, SMI_HANDLER_SIGNATURE);
    Size      += sizeof (SMM_CORE_SMI_HANDLER_STRUCTURE) + GET_OCCUPIED_SIZE (
                                                             SmiHandler->
                                                               ContextSize,
                                                             sizeof (UINT64)
                                                             );
  ...
  )
{
  ...
  mSmmRootSmiDatabaseSize = GetSmmSmiDatabaseSize (
                              mSmmCoreRootSmiEntryList
                              );
  mSmmSmiDatabaseSize         = GetSmmSmiDatabaseSize (mSmmCoreSmiEntryList);
  mSmmHardwareSmiDatabaseSize = GetSmmSmiDatabaseSize (
                                  mSmmCoreHardwareSmiEntryList
                                  );

  return mSmmImageDatabaseSize + mSmmSmiDatabaseSize + mSmmRootSmiDatabaseSize +
  ...
  )
{
  ...
  for (Index = 0; Index < mImageStructCount; Index++) {
    ...
    if (sizeof (SMM_CORE_IMAGE_DATABASE_STRUCTURE) + GET_OCCUPIED_SIZE (
                                                       mImageStruct[Index].
                                                         PdbStringSize,
                                                       sizeof (UINT64)
                                                       ) > ExpectedSize - Size)
    ...
    ImageStruct->Header.Length    =
      (UINT32)(sizeof (SMM_CORE_IMAGE_DATABASE_STRUCTURE) + GET_OCCUPIED_SIZE (
                                                              mImageStruct[Index
                                                              ].PdbStringSize,
                                                              sizeof (UINT64)
                                                              ));
    ...
    if (mImageStruct[Index].PdbStringSize != 0) {
      ...
      CopyMem (
        (VOID *)((UINTN)ImageStruct + ImageStruct->PdbStringOffset),
        mImageStruct[Index].PdbString,
  ...
  *Count           = 0;
       ...
       ListEntry = ListEntry->ForwardLink)
  {
    ...
    SmiHandlerStruct->Length =
      (UINT32)(sizeof (SMM_CORE_SMI_HANDLER_STRUCTURE) + GET_OCCUPIED_SIZE (
                                                           SmiHandler->
                                                             ContextSize,
                                                           sizeof (UINT64)
                                                           ));
    ...
    SmiHandlerStruct->ImageRef   = AddressToImageRef (
                                     (UINTN)SmiHandler->Handler
                                     );
    SmiHandlerStruct->ContextBufferSize = (UINT32)SmiHandler->ContextSize;
    if (SmiHandler->ContextSize != 0) {
      SmiHandlerStruct->ContextBufferOffset =
        sizeof (SMM_CORE_SMI_HANDLER_STRUCTURE);
      CopyMem (
        (UINT8 *)SmiHandlerStruct +
        SmiHandlerStruct->ContextBufferOffset,
        SmiHandler->Context,
        SmiHandler->ContextSize
        );
    ...
    Size += sizeof (SMM_CORE_SMI_HANDLER_STRUCTURE) +
            GET_OCCUPIED_SIZE (SmiHandler->ContextSize, sizeof (UINT64));
    SmiHandlerStruct =
      (SMM_CORE_SMI_HANDLER_STRUCTURE *)((UINTN)SmiHandlerStruct +
                                         SmiHandlerStruct->Length);
    *Count = *Count + 1;
  }
  ...
  )
{
       ...
       ListEntry = ListEntry->ForwardLink)
  {
    ...
    SmiHandlerSize = GetSmmSmiHandlerDataOnSmiEntry (
                       SmiEntry,
                       (UINT8 *)SmiStruct + SmiStruct->Header.Length,
                       ExpectedSize - Size,
                       &SmiHandlerCount
                       );
    ...
    SmiStruct                 = (VOID *)((UINTN)SmiStruct +
                                         SmiStruct->Header.Length);
  }
  ...
  )
{
  ...
  SmmRootSmiDatabaseSize = GetSmmSmiDatabaseData (
                             mSmmCoreRootSmiEntryList,
  ...
  SmmSmiDatabaseSize = GetSmmSmiDatabaseData (
  ...
  SmmHardwareSmiDatabaseSize = GetSmmSmiDatabaseData (
                                 mSmmCoreHardwareSmiEntryList,
  ...
  )
{
  ...
  mSmiHandlerProfileDatabase     = AllocatePool (
  ...
  )
{
  ...
  SmiHandlerProfileParameterGetInfo->DataSize =
    mSmiHandlerProfileDatabaseSize;
...
SmiHandlerProfileHandlerGetDataByOffset (
  IN SMI_HANDLER_PROFILE_PARAMETER_GET_DATA_BY_OFFSET  *
  ...
  )
{
  ...
  if (!SmmIsBufferOutsideSmmValid (
         (UINTN)SmiHandlerProfileGetDataByOffset.DataBuffer,
         (UINTN)SmiHandlerProfileGetDataByOffset.DataSize
         ))
  {
    DEBUG ((
      DEBUG_ERROR,
    ...
    SmiHandlerProfileParameterGetDataByOffset->Header.ReturnStatus =
      (UINT64)(INT64)(INTN)EFI_ACCESS_DENIED;
    goto Done;
  }

  SmiHandlerProfileCopyData (
    (VOID *)(UINTN)SmiHandlerProfileGetDataByOffset.DataBuffer,
    &SmiHandlerProfileGetDataByOffset.DataSize,
    &SmiHandlerProfileGetDataByOffset.DataOffset
    );
  CopyMem (
    SmiHandlerProfileParameterGetDataByOffset,
    &SmiHandlerProfileGetDataByOffset,
  ...
  )
{
  ...
  SmiHandlerProfileParameterHeader =
    (SMI_HANDLER_PROFILE_PARAMETER_HEADER *)((UINTN)CommBuffer);
  ...
  switch (SmiHandlerProfileParameterHeader->Command) {
    case SMI_HANDLER_PROFILE_COMMAND_GET_INFO:
      ...
      SmiHandlerProfileHandlerGetInfo (
        (SMI_HANDLER_PROFILE_PARAMETER_GET_INFO *)(UINTN)CommBuffer
        );
      break;
    case SMI_HANDLER_PROFILE_COMMAND_GET_DATA_BY_OFFSET:
      ...
      SmiHandlerProfileHandlerGetDataByOffset (
        (SMI_HANDLER_PROFILE_PARAMETER_GET_DATA_BY_OFFSET *)(UINTN)CommBuffer
        );
      break;
    default:
      break;
  ...
  )
{
  ...
  SmiHandlerUsbContext = AllocatePool (
  ...
  *SmiHandlerUsbContextSize =
    sizeof (SMI_HANDLER_PROFILE_USB_REGISTER_CONTEXT) + DevicePathSize;
  ...
  )
{
  ...
  SmiHandlerSwContext = AllocatePool (
  ...
  )
{
  ...
  if (Context != NULL) {
    if (CompareGuid (HandlerGuid, &gEfiSmmUsbDispatch2ProtocolGuid)) {
      SmiHandler->Context = ConvertSmiHandlerUsbContext (
                              Context,
                              ContextSize,
                              &SmiHandler->ContextSize
                              );
    } else if (CompareGuid (HandlerGuid, &gEfiSmmSwDispatch2ProtocolGuid)) {
      SmiHandler->Context = ConvertSmiHandlerSwContext (
                              Context,
                              ContextSize,
                              &SmiHandler->ContextSize
                              );
  ...
  )
{
  ...
  if (Context != NULL) {
    if (CompareGuid (HandlerGuid, &gEfiSmmUsbDispatch2ProtocolGuid)) {
      SearchContext = ConvertSmiHandlerUsbContext (
                        Context,
                        ContextSize,
                        &SearchContextSize
                        );
    } else if (CompareGuid (HandlerGuid, &gEfiSmmSwDispatch2ProtocolGuid)) {
      SearchContext = ConvertSmiHandlerSwContext (
                        Context,
                        ContextSize,
                        &SearchContextSize
                        );
  ...
  for (Link = Head->ForwardLink; Link != Head; Link = Link->ForwardLink) {
    ...
    if (SmiHandler->Handler == Handler) {
      if ((SearchContext == NULL) ||
          ((SearchContextSize == SmiHandler->ContextSize) && (CompareMem (
                                                                SearchContext,
                                                                SmiHandler->
                                                                  Context,
                                                                SearchContextSize
