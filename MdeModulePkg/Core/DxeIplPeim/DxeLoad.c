  ...
  )
{
  ...
  if (BootMode == BOOT_ON_S3_RESUME) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      //
      if (!EFI_ERROR (Status)) {
        Status = PeiCapsuleOnDisk->LoadCapsuleOnDisk (
                                     PeiServices,
                                     PeiCapsuleOnDisk
                                     );
      }
      ...
      NULL)
  {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      if (!EFI_ERROR (Status) && ValidateMemoryTypeInfoVariable (
                                   MemoryData,
                                   DataSize
                                   ))
      {
  ...
  )
{
  ...
  if (((SectionAttribute & EFI_GUIDED_SECTION_PROCESSING_REQUIRED) != 0) &&
      (OutputBufferSize > 0))
  {
    //
