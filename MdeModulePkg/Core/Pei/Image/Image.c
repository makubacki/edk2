  ...
  )
{
  ...
  for (Index = 0; Index < NumberOfSections; Index++) {
    ...
    if ((SectionHeader.Characteristics & EFI_IMAGE_SCN_CNT_CODE) == 0) {
      ...
      // header
      // that doesn't point to code section in image header, as well as ImageBase field of image header. A notable thing
      // is
      // that for PEIM, the value in ImageBase field may not be equal to the value in PointerToRelocations &
      // PointerToLineNumbers because
      // for XIP PEIM, ImageBase field holds the image base address running on the Flash. And PointerToRelocations &
      // PointerToLineNumbers
      // hold the image base address when it is shadow to the memory. And there is an assumption that when the feature
      // is enabled, if a
      // module is assigned a loading address by tools, PointerToRelocations & PointerToLineNumbers fields should NOT be
      // Zero, or
      ...
      if (ValueInSectionHeader != 0) {
        ...
        if ((INT64) PcdGet64 (PcdLoadModuleAtFixAddressEnable) > 0) {
          //
          // When LMFA feature is configured as Load Module at Fixed Absolute Address mode, PointerToRelocations &
          ...
          //
          // When LMFA feature is configured as Load Module at Fixed offset mode, PointerToRelocations &
          // PointerToLineNumbers field
          // hold the offset relative to a platform-specific top address.
          //
          FixLoadingAddress =
            (EFI_PHYSICAL_ADDRESS) (Private->LoadModuleAtFixAddressTopAddress + (INT64) ValueInSectionHeader);
        }
  ...
  )
{
  ...
  if (ImageContext.RelocationsStripped && (Private->PeiMemoryInstalled) &&
    ...
    DEBUG (
          (EFI_D_INFO|EFI_D_LOAD, "The image at 0x%08x without reloc section can't be loaded into memory\n",
           (UINTN) Pe32Data)
          );
  }
  ...
  if ((!ImageContext.RelocationsStripped) && (Private->PeiMemoryInstalled) &&
    ...
    //
    if (ImageContext.IsTeImage) {
      AlignImageSize = ImageContext.ImageSize + ((EFI_TE_IMAGE_HEADER *) Pe32Data)->StrippedSize -
  ...
  if (EFI_ERROR (Status)) {
    if (ImageContext.ImageError == IMAGE_ERROR_INVALID_SECTION_ALIGNMENT) {
      DEBUG (
            (DEBUG_ERROR, "PEIM Image Address 0x%11p doesn't meet with section alignment 0x%x.\n",
  ...
  )
{
  ...
  *EntryPoint = ImageEntryPoint;
  ...
  if (Machine != EFI_IMAGE_MACHINE_IA64) {
    DEBUG (
            (EFI_D_INFO | EFI_D_LOAD, "Loading PEIM at 0x%11p EntryPoint=0x%11p ", (VOID *) (UINTN) ImageAddress,
    ...
    DEBUG (
            (EFI_D_INFO | EFI_D_LOAD, "Loading PEIM at 0x%11p EntryPoint=0x%11p ", (VOID *) (UINTN) ImageAddress,
             (VOID *) (UINTN) (*(UINT64 *) (UINTN) *EntryPoint))
            );
  }
