...
//
// Only used when load module at fixed address feature is enabled. True means the memory is alreay successfully
// allocated
  ...
  )
{
  ...
  if (!gLoadFixedAddressCodeMemoryReady) {
    ...
    RuntimeCodeBase =
      (EFI_PHYSICAL_ADDRESS) (gLoadModuleAtFixAddressConfigurationTable.DxeCodeTopAddress -
  ...
  )
{
  ...
  //
  // If Loading Module At Fixed Address feature is enabled. try to allocate memory with Runtime code & Boot time code
  // type
  //
    ...
    if (ChangingType) {
      ...
      if ((UINT32) NewType < EfiMaxMemoryType) {
        if ((Start >= mMemoryTypeStatistics[NewType].BaseAddress &&
             Start <= mMemoryTypeStatistics[NewType].MaximumAddress) ||
            (Start >= mDefaultBaseAddress && Start <= mDefaultMaximumAddress)) {
  ...
  )
{
  ...
  for ( ; MemoryMap != MemoryMapDescriptor; MemoryMap = NEXT_MEMORY_DESCRIPTOR (MemoryMap, DescriptorSize)) {
    ...
    //
    if (MemoryMap->PhysicalStart + EFI_PAGES_TO_SIZE ((UINTN) MemoryMap->NumberOfPages) ==
    ...
    //
    if (MemoryMap->PhysicalStart - EFI_PAGES_TO_SIZE ((UINTN) MemoryMapDescriptor->NumberOfPages) ==
        MemoryMapDescriptor->PhysicalStart) {
      //
      // Merge MemoryMapDescriptor into MemoryMap
      //
