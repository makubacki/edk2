  ...
  )
{
  ...
  if (CompareGuid (Entry->ProtocolGuid, &gEfiRuntimeArchProtocolGuid)) {
    ...
    //
    for (Link = gRuntimeTemplate.ImageHead.ForwardLink;
         Link != &gRuntimeTemplate.ImageHead;
         Link = TempLinkNode.ForwardLink) {
    ...
    //
    for (Link = gRuntimeTemplate.EventHead.ForwardLink;
         Link != &gRuntimeTemplate.EventHead;
         Link = TempLinkNode.ForwardLink) {
      CopyMem (&TempLinkNode, Link, sizeof (LIST_ENTRY));
