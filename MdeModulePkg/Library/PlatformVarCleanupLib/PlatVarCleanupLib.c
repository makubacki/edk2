  ...
  )
{
  ...
  if (Action == EFI_BROWSER_ACTION_CHANGING) {
    ...
    if ((QuestionId >= USER_VARIABLE_QUESTION_ID) &&
        (QuestionId < USER_VARIABLE_QUESTION_ID + MAX_USER_VARIABLE_COUNT)) {
      if (Value->b) {
        //
  ...
  )
{
  ...
  if ((Type >= VarCleanupMax) ||
      ((Flag & ((VAR_ERROR_FLAG)(VAR_ERROR_FLAG_SYSTEM_ERROR & VAR_ERROR_FLAG_USER_ERROR))) == 0)) {
    return EFI_INVALID_PARAMETER;
  }
