...
#define VAR_CHECK_HII_VARIABLE_FROM_LINK(a)  CR ( \
                                               a, \
                                               VAR_CHECK_HII_VARIABLE_NODE, \
                                               Link, \
        ...
        while ((UINTN)IfrOpCodeHeader < ((UINTN)HiiPackageHeader + HiiPackageHeader->Length)) {
          switch (IfrOpCodeHeader->OpCode) {
            ...
            case EFI_IFR_GUID_OP:
              if (CompareGuid (
                    (EFI_GUID *)((UINTN)IfrOpCodeHeader + sizeof (EFI_IFR_OP_HEADER)),
  ...
  )
  {
    ...
    while ((UINTN)HiiPackageListHeader < ((UINTN)HiiDatabase + HiiDatabaseSize)) {
      ...
      HiiPackageListHeader =
        (EFI_HII_PACKAGE_LIST_HEADER *)((UINTN)HiiPackageListHeader + HiiPackageListHeader->PackageLength);
    }
  ...
  switch (HiiQuestion1->OpCode) {
    case EFI_IFR_ONE_OF_OP:
      DEBUG ((
        ...
        (HiiQuestion1->BitFieldStore ? "bit level" : "byte level")
        ));
      //
  ...
  )
{
  ...
  if (HiiVariableNode == NULL) {
    ...
    HiiVariableNode->HiiQuestionArray =
      InternalVarCheckAllocateZeroPool (IfrEfiVarStore->Size * 8 * sizeof (VAR_CHECK_HII_QUESTION_HEADER *));
    ...
    case EFI_HII_PACKAGE_FORMS:
      ...
      while ((UINTN)IfrOpCodeHeader < (UINTN)HiiPackageHeader + HiiPackageHeader->Length) {
        switch (IfrOpCodeHeader->OpCode) {
          ...
          case EFI_IFR_ORDERED_LIST_OP:
            HiiVariableNode =
              FindHiiVariableNodeByVarStoreId (((EFI_IFR_ONE_OF *)IfrOpCodeHeader)->Question.VarStoreId);
            if ((HiiVariableNode == NULL) ||
                //
