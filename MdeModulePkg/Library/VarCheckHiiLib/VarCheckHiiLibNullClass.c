  ...
  switch (HiiQuestion->OpCode) {
    ...
    case EFI_IFR_ORDERED_LIST_OP:
      ...
      for (Index = 0; Index < MaxContainers; Index++) {
        OneData = 0;
        CopyMem (&OneData,
          (UINT8 *)Data + HiiQuestion->VarOffset + HiiQuestion->StorageWidth * Index,
        ...
        if ((UINTN)Ptr >= ((UINTN)HiiQuestion + HiiQuestion->Length)) {
          ...
          DEBUG_CODE (VarCheckHiiInternalDumpHex (2, 0, HiiQuestion->StorageWidth * MaxContainers,
                        (UINT8 *)Data + HiiQuestion->VarOffset););
          DEBUG_CODE (VarCheckHiiInternalDumpHex (2, 0, HiiQuestion->Length, (UINT8 *)HiiQuestion););
          return FALSE;
        }
      }
  ...
  **/
  ...
  )
  {
    ...
    DEBUG ((DEBUG_INFO, "    OpCode        - 0x%02x (%a) (%a)\n", HiiQuestion->OpCode,
      HiiOpCodeToStr (HiiQuestion->OpCode), (HiiQuestion->BitFieldStore ? "bit level" : "byte level")));
