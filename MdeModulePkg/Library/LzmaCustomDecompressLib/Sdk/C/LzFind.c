...
static void
LzInWindow_Free (
  CMatchFinder *p, ISzAllocPtr alloc
  )
{
  if (!p->directInput) {
...
static int
LzInWindow_Create (
  CMatchFinder *p, UInt32 keepSizeReserv, ISzAllocPtr alloc
  )
{
...
MatchFinder_Normalize3 (
  UInt32 subValue, CLzRef *items, size_t numItems
  )
{
  size_t  i;
...
static UInt32 *
Hc_GetMatchesSpec (
  unsigned lenLimit, UInt32 curMatch, UInt32 pos, const Byte *cur, CLzRef *son,
  ...
  )
{
  ...
  for ( ; ;) {
    ...
    {
      CLzRef  *pair = son +
                      ((size_t)(_cyclicBufferPos - delta + ((delta > _cyclicBufferPos) ? _cyclicBufferSize : 0)) << 1);
      const Byte  *pb   = cur - delta;
...
#define GET_MATCHES_FOOTER(offset, maxLen) \
  offset = (unsigned)(GetMatchesSpec1 ( \
                        (UInt32)lenLimit, \
                        curMatch, \
                        MF_PARAMS (p), \
                        distances + offset, \
                        (UInt32)maxLen \
                        ) - distances);MOVE_POS_RET;
...
Bt2_MatchFinder_GetMatches (
  CMatchFinder *p, UInt32 *distances
  )
{
  unsigned  offset;
...
MatchFinder_CreateVTable (
  CMatchFinder *p, IMatchFinder *vTable
  )
{
