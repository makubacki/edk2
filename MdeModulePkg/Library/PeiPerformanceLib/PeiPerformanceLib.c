...
#define  PEI_MAX_RECORD_SIZE  \
              (sizeof (FPDT_DUAL_GUID_STRING_EVENT_RECORD) + STRING_SIZE)
  ...
  )
{
  ...
  while (GuidHob != NULL) {
    ...
    *PeiPerformanceLogHeader =
      (FPDT_PEI_EXT_PERF_HEADER *)PeiFirmwarePerformance;
  ...
  )
{
  ...
  return (RETURN_STATUS)InsertFpdtRecord (
                          Handle,
                          NULL,
                          String,
                          TimeStamp,
