  ...
  )
{
  ...
  if (BmIsKeyOptionVariable (Name, Guid, &OptionNumber)) {
    ...
    if (BmIsKeyOptionValid (KeyOption, KeyOptionSize)) {
      ...
      CopyMem (
        &Param->KeyOptions[Index + 1],
        &Param->KeyOptions[Index],
    ...
    for (KeyIndex = 0; KeyIndex < Hotkey->CodeCount; KeyIndex++) {
      ...
      if (!EFI_ERROR (Status)) {
        ...
        DEBUG ((
          ...
          Hotkey->KeyData[KeyIndex].Key.UnicodeChar,
          Status
          ));
      }
    }
    ...
    case 1:
      if (KeyOption->KeyData.Options.ControlPressed) {
        BmGenerateKeyShiftState (
          Depth + 1,
          KeyOption,
          KeyShiftState | EFI_RIGHT_CONTROL_PRESSED,
          KeyShiftStates,
          KeyShiftStateCount
          );
        BmGenerateKeyShiftState (
    ...
    case 2:
      if (KeyOption->KeyData.Options.AltPressed) {
        BmGenerateKeyShiftState (
          Depth + 1,
          KeyOption,
          KeyShiftState | EFI_RIGHT_ALT_PRESSED,
          KeyShiftStates,
          KeyShiftStateCount
          );
        BmGenerateKeyShiftState (
          Depth + 1,
          KeyOption,
          KeyShiftState | EFI_LEFT_ALT_PRESSED,
          KeyShiftStates,
          KeyShiftStateCount
          );
      } else {
    ...
    case 3:
      if (KeyOption->KeyData.Options.LogoPressed) {
        BmGenerateKeyShiftState (
          Depth + 1,
          KeyOption,
          KeyShiftState | EFI_RIGHT_LOGO_PRESSED,
          KeyShiftStates,
          KeyShiftStateCount
          );
        BmGenerateKeyShiftState (
          Depth + 1,
          KeyOption,
          KeyShiftState | EFI_LEFT_LOGO_PRESSED,
          KeyShiftStates,
          KeyShiftStateCount
          );
      } else {
  ...
  if (BootOptionSupport != NULL) {
    if ((*BootOptionSupport & EFI_BOOT_OPTION_SUPPORT_KEY)  != 0) {
      mBmHotkeySupportCount =
        ((*BootOptionSupport & EFI_BOOT_OPTION_SUPPORT_COUNT) >> LowBitSet32 (EFI_BOOT_OPTION_SUPPORT_COUNT));
    }
    ...
    for (Index = 0; Match && (Index < Hotkey->CodeCount); Index++) {
      ...
      if (
          ...
          (BmBitSet (
             ...
             ) != KeyOption.KeyData.Options.AltPressed) ||
          (BmBitSet (
             ShiftState,
             EFI_RIGHT_LOGO_PRESSED | EFI_LEFT_LOGO_PRESSED
             ) != KeyOption.KeyData.Options.LogoPressed) ||
