  ...
  )
{
  ...
  *Progress = Configuration;
  ...
  if (CompareMem (NewBmmData->BootOptionDel, OldBmmData->BootOptionDel, sizeof (NewBmmData->BootOptionDel)) != 0) {
    for (Index = 0;
         ((Index < BootOptionMenu.MenuNumber) &&
          (Index < (sizeof (NewBmmData->BootOptionDel) / sizeof (NewBmmData->BootOptionDel[0]))));
         Index++) {
  ...
  if (CompareMem (
    ...
    for (Index = 0;
         ((Index < DriverOptionMenu.MenuNumber) &&
          (Index < (sizeof (NewBmmData->DriverOptionDel) / sizeof (NewBmmData->DriverOptionDel[0]))));
         Index++) {
  ...
  )
{
  ...
  if (Action == EFI_BROWSER_ACTION_CHANGING) {
    ...
    } else if ((QuestionId >= DRIVER_OPTION_DEL_QUESTION_ID) &&
               (QuestionId < DRIVER_OPTION_DEL_QUESTION_ID + MAX_MENU_NUMBER)) {
      if (Value->b) {
    ...
    } else if ((QuestionId >= CON_OUT_DEVICE_QUESTION_ID) &&
               (QuestionId < CON_OUT_DEVICE_QUESTION_ID + MAX_MENU_NUMBER)) {
      UpdateConsoleContent (L"ConOut", CurrentFakeNVMap);
    } else if ((QuestionId >= CON_ERR_DEVICE_QUESTION_ID) &&
               (QuestionId < CON_ERR_DEVICE_QUESTION_ID + MAX_MENU_NUMBER)) {
  ...
  switch (Private->BmmPreviousPageId) {
    ...
    case FORM_BOOT_DEL_ID:
      ASSERT (
        ...
        (sizeof (CurrentFakeNVMap->BootOptionDel) / sizeof (CurrentFakeNVMap->BootOptionDel[0]))
        );
    ...
    case FORM_DRV_DEL_ID:
      ASSERT (
        ...
        (sizeof (CurrentFakeNVMap->DriverOptionDel) / sizeof (CurrentFakeNVMap->DriverOptionDel[0]))
        );
  ...
  )
{
  ...
  //
  StartGuidLabel =
    (EFI_IFR_GUID_LABEL *)HiiCreateGuidOpCode (StartOpCodeHandle, &gEfiIfrTianoGuid, NULL, sizeof (EFI_IFR_GUID_LABEL));
...

{
  ...
  Ptr =
    AllocateZeroPool (
      sizeof (BM_LOAD_CONTEXT) + sizeof (BM_FILE_CONTEXT) + sizeof (BM_HANDLE_CONTEXT) + sizeof (BM_MENU_ENTRY)
      );
  ASSERT (Ptr != NULL);
