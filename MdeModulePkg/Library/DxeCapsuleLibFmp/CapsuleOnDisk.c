  ...
  *CapsuleNameNum = 0;
  ...
  if (((UINTN) CapsuleNameBufStart & BIT0) != 0) {
    CapsuleNameBufStart = AllocateCopyPool (
                                           CapsuleHeader->CapsuleImageSize - CapsuleHeader->HeaderSize,
                                           CapsuleNameBufStart
                                           );
      ...
      if (SubStrCmpResult < 0) {
        ...
        if (NewFileName[IndexNew] == L'\0') {
          if (ListedFileName[IndexListed] != L'\0' ||
              (StrnCmp (NewFileNameExtension, ListedFileNameExtension, MAX_FILE_NAME_LEN) < 0)) {
            break;
          }
        }
      }
  ...
  )
{
  ...
  if (DivU64x32 (TempCodFileSize, BlockIo->Media->BlockSize) > BlockIo->Media->LastBlock) {
    ...
    DEBUG (
         (DEBUG_ERROR, "RelocationDev BlockSize = %x LastBlock = %x\n", BlockIo->Media->BlockSize,
  ...
  )
{
  ...
  if (PcdGetBool (PcdCapsuleInRamSupport)) {
    ...
    DEBUG (
          (DEBUG_INFO, "Reallcoate all Capsule on Disks to %s in RootDir.\n",
           (CHAR16 *) PcdGetPtr (PcdCoDRelocationFileName))
          );
