  ...
  )
{
  ...
  mSecurity2Table = ReallocatePool (
                                    ...
                                    (mMaxNumberOfSecurity2Handler + SECURITY_HANDLER_TABLE_SIZE) *
  ...
  )
{
  ...
  for (Index = 0; Index < mNumberOfSecurity2Handler; Index++) {
    ...
    // If FileBuffer is NULL, the input is not Image, which will be handled by EFI_AUTH_NONE_IMAGE_OPERATION_MASK
    // operation.
