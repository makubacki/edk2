  ...
  )
{
  ...
  while (GuidHob != NULL) {
    ...
    //
    Initialized = TRUE;
    if ((SerialPortReadRegister (
           SerialRegisterBase,
           R_UART_LCR,
           MmioEnable,
           RegisterStride
           ) & 0x3F) != (PcdGet8 (PcdSerialLineControl) & 0x3F))
    {
      Initialized = FALSE;
    }

    Value = (UINT8)(SerialPortReadRegister (
                      SerialRegisterBase,
                      R_UART_LCR,
                      MmioEnable,
                      RegisterStride
    ...
    CurrentDivisor =  SerialPortReadRegister (
                        SerialRegisterBase,
                        R_UART_BAUD_HIGH,
                        MmioEnable,
                        RegisterStride
                        ) << 8;
    CurrentDivisor |= (UINT32)SerialPortReadRegister (
                                SerialRegisterBase,
                                R_UART_BAUD_LOW,
                                MmioEnable,
                                RegisterStride
  ...
  )
{
  ...
  while (Count < mUartCount) {
    ...
    while (BytesLeft != 0) {
      ...
      //
      for (Index = 0; Index < FifoSize && BytesLeft != 0; Index++, BytesLeft--,
           DataBuffer++)
      {
        //
  ...
  )
{
  ...
  while (Count < mUartCount) {
    ...
    Mcr = (UINT8)(SerialPortReadRegister (
                    BaseAddress,
                    R_UART_MCR,
                    UseMmio,
                    Stride
    ...
    for (Result = 0; BytesLeft-- != 0; Result++, DataBuffer++) {
      ...
      //
      *DataBuffer = SerialPortReadRegister (
                      BaseAddress,
                      R_UART_RXBUF,
                      UseMmio,
                      Stride
  ...
  )
{
  ...
  while (Count < mUartCount) {
    ...
    //
    if ((SerialPortReadRegister (BaseAddress, R_UART_LSR, UseMmio, Stride) &
         B_UART_LSR_RXRDY) != 0)
    {
      if (PcdGetBool (PcdSerialUseHardwareFlowControl)) {
        //
