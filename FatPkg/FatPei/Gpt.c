  ...
  )
{
  ...
  //
  // |    Block0    |    Block1    |Block2 ~ FirstUsableLBA - 1|FirstUsableLBA, ... ,LastUsableLBA|LastUsableLBA+1 ~
  // LastBlock-1|  LastBlock  |
  // |Protective MBR|Primary Header|Entry Array(At Least 16384)|             Partition            | Entry Array(At Least
  // 16384) |BackUp Header|
  //
  ...
  if ((PartHdr->MyLBA != Lba) ||
      ...
      (PartHdr->LastUsableLBA  >
       ParentBlockDev->LastBlock - 1 - EFI_SIZE_TO_BLOCKS (EFI_GPT_PART_ENTRY_MIN_SIZE, ParentBlockDev->BlockSize)) ||
  ...
  )
{
  ...
  PartitionEntryStatus =
    (EFI_PARTITION_ENTRY_STATUS *) AllocatePages (
                                                                        EFI_SIZE_TO_PAGES (
                                                                                                                 PartHdr
                                                                                                                   ->
...
EXIT:
  ...
  if (PartitionEntryStatus != NULL) {
    FreePages (
              PartitionEntryStatus,
              EFI_SIZE_TO_PAGES (PartHdr->NumberOfPartitionEntries * sizeof (EFI_PARTITION_ENTRY_STATUS))
              );
  }
