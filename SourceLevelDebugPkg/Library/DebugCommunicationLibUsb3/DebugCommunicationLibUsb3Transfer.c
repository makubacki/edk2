  ...
  )
{
  ...
  for (Index = 0; Index < Handle->EventRing.TrbNumber; Index++) {
    ...
    TRBPtr = (TRB_TEMPLATE *)(UINTN)(EvtTrb->TRBPtrLo | LShiftU64 (
                                                          (UINT64)EvtTrb->
                                                            TRBPtrHi,
                                                          32
                                                          ));
    ...
    if (IsTrbInTrsRing ((TRANSFER_RING *)(UINTN)(Urb->Ring), TRBPtr)) {
      CheckedUrb = Urb;
    } else if (IsTrbInTrsRing (
                 (TRANSFER_RING *)(UINTN)(Handle->UrbIn.Ring),
                 TRBPtr
                 ))
    {
      //
      ...
      CopyMem (
        (VOID *)(UINTN)Handle->Data,
        (VOID *)(UINTN)Handle->UrbIn.Data,
        Handle->DataCount
        );
      //
...
EXIT:
  ...
  if ((XhcDequeue & (~0x0F)) !=
      ((UINT64)(UINTN)Handle->EventRing.EventRingDequeue & (~0x0F)))
  {
    //
    // Some 3rd party XHCI external cards don't support single 64-bytes width register access,
    // So divide it to two 32-bytes width register access.
    ...
    XhcWriteDebugReg (
      ...
      XHC_DC_DCERDP,
      XHC_LOW_32BIT (
        Handle->EventRing.EventRingDequeue
        )
      );
    XhcWriteDebugReg (
      Handle,
