...
GLOBAL_REMOVE_IF_UNREFERENCED UINTN  mVectorHandoffInfoCount = sizeof (mVectorHandoffInfoDebugAgent) /
  ...
  )
{
  ...
  if (CheckSum != Mailbox->CheckSum && CheckSum != Mailbox->ToBeCheckSum) {
    ...
    DEBUG (
          (EFI_D_ERROR, "DebugAgent: CheckSum = %x, Mailbox->CheckSum = %x, Mailbox->ToBeCheckSum = %x\n", CheckSum,
           Mailbox->CheckSum, Mailbox->ToBeCheckSum)
          );
    CpuDeadLoop ();
  }
}
  ...
  )
{
  ...
  switch (Index) {
    ...
    case DEBUG_MAILBOX_EXCEPTION_BUFFER_POINTER_INDEX:
      Mailbox->ToBeCheckSum = Mailbox->CheckSum +
                              CalculateSum8 ((UINT8 *) &Mailbox->ExceptionBufferPointer, sizeof (UINTN))
                              - CalculateSum8 ((UINT8 *) &Value, sizeof (UINTN));
    ...
    case DEBUG_MAILBOX_DEBUG_TIMER_FREQUENCY:
      Mailbox->ToBeCheckSum = Mailbox->CheckSum + CalculateSum8 (
                                                              (UINT8 *) &Mailbox->DebugTimerFrequency,
                                                              sizeof (UINT32)
                                                              )
  ...
  )
{
  ...
  if (DebugAgentReadBuffer (
                           ...
                           sizeof (DEBUG_PACKET_HEADER) - OFFSET_OF (DEBUG_PACKET_HEADER, Command),
                           READ_PACKET_TIMEOUT
                           ) == 0) {
    //
    // Timeout occur, exit
  ...
  )
{
  ...
  while (TRUE) {
    ...
    Received = DebugAgentReadBuffer (
                           ...
                           DEBUG_PACKET_HEADER,
                           Length
                           ) + sizeof (DebugHeader->Length) - sizeof (DebugHeader->StartSymbol),
                                     Timeout
                                     );
    if (Received == 0) {
    ...
    if (DebugHeader->Length < sizeof (DEBUG_PACKET_HEADER)) {
      ...
      Received =
        DebugAgentReadBuffer (
                                        ...
                                        (UINT8) (DebugHeader->Length - OFFSET_OF (DEBUG_PACKET_HEADER, SequenceNo)),
                                        Timeout
                                        );
      if (Received == 0) {
  ...
  )
{
  ...
  while (RetryCount > 0) {
    ...
    if (DebugHeader->Command == DEBUG_COMMAND_GO &&
        (DebugHeader->SequenceNo == HostSequenceNo || Command == DEBUG_COMMAND_INIT_BREAK)) {
      //
      // Received Old GO
      //
      if (Command == DEBUG_COMMAND_INIT_BREAK) {
  ...
  )
{
  ...
  if (IncompatibilityFlag) {
    //
    // If the incompatible Debug Packet received, the HOST should be running transfer protocol before
    ...
    switch (DebugHeader->Command) {
      ...
      case DEBUG_COMMAND_GET_REVISION:
        ...
        Status =
          SendDataResponsePacket (
                                          ...
                                          (UINT16) sizeof (DEBUG_DATA_RESPONSE_GET_REVISION),
                                          DebugHeader
                                          );
        break;
      ...
      case DEBUG_COMMAND_GET_EXCEPTION:
        ...
        Status = SendDataResponsePacket (
                                      (UINT8 *) &Exception,
                                      (UINT16) sizeof (DEBUG_DATA_RESPONSE_GET_EXCEPTION),
                                      DebugHeader
                                      );
        break;
      ...
      case DEBUG_COMMAND_SEARCH_SIGNATURE:
        ...
        if ((SearchSignature->Alignment != 0) &&
          ...
          if (SearchSignature->Positive) {
            ...
            for (
                 Data64 =
                   ALIGN_VALUE ((UINTN) SearchSignature->Start - SearchSignature->Alignment,
  ...
  )
{
  ...
  if (mSkipBreakpoint) {
    //
    // If Skip Breakpoint flag is set, means communication is disturbed by hardware SMI, we need to ignore the break
    // points in SMM
    //
