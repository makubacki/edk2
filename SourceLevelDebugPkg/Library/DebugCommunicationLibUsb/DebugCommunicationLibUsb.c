  ...
  )
{
  ...
  UsbDebugPortMemoryBase = 0xFFFFFC00 & PciRead32 (
                                                 PcdGet32 (
                                                           ...
                                                           ) + PCI_BASE_ADDRESSREG_OFFSET + Handle->DebugPortBarNumber * 4
                                                 );
  ...
  //
  // If the owner and in_use bit is not set, it means system is doing cold/warm boot or EHCI host controller is reset by
  // system software.
  //
  ...
  )
{
  ...
  UsbDebugPortRegister =
    (USB_DEBUG_PORT_REGISTER *) ((UINTN) UsbDebugPortHandle->UsbDebugPortMemoryBase +
  ...
  while ((Total < NumberOfBytes)) {
    ...
    //
    // If received a NAK_PID on write transaction, it means the usb debug device is busy and can not handle this
    // transaction.
