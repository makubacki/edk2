  ...
  )
{
  ...
  Status = Host->ReceiveResponse (
                   Host,
                   MMC_RESPONSE_TYPE_R2,
                   (UINT32 *)&(MmcHostInstance->CardInfo.CIDData)
                   );
  if (EFI_ERROR (Status)) {
    DEBUG ((
      DEBUG_ERROR,
  ...
  RCA                           = MmcHostInstance->CardInfo.RCA <<
                                  RCA_SHIFT_OFFSET;
  ...
  Status = Host->ReceiveResponse (
                   Host,
                   MMC_RESPONSE_TYPE_R2,
                   (UINT32 *)&(MmcHostInstance->CardInfo.CSDData)
                   );
  if (EFI_ERROR (Status)) {
    DEBUG ((
      DEBUG_ERROR,
  ...
  // Fetch ECSD
  MmcHostInstance->CardInfo.ECSDData = AllocatePages (
                                         EFI_SIZE_TO_PAGES (
                                           sizeof (ECSD)
                                           )
                                         );
  ...
  Status = Host->ReadBlockData (
                   Host,
                   0,
                   512,
                   (UINT32 *)MmcHostInstance->CardInfo.ECSDData
                   );
  if (EFI_ERROR (Status)) {
    DEBUG ((
      DEBUG_ERROR,
  ...
  Media->ReadOnly  =
    MmcHostInstance->CardInfo.CSDData.PERM_WRITE_PROTECT;
  ...
  )
{
  ...
  if (MmcHostInstance->CardInfo.CardType == SD_CARD_2_HIGH) {
    ...
    NumBlocks = (CardSize + 1) * (1 << (MMC_CSD_GET_DEVICESIZEMULT (Response) +
                                        2));
    BlockSize = 1 << MMC_CSD_GET_READBLLEN (Response);
  }
