  ...
  )
{
  ...
  DEBUG (
        (EFI_D_INFO, "    specVersion        - %d.%d%d\n", TcgEfiSpecIdEventStruct->specVersionMajor,
         TcgEfiSpecIdEventStruct->specVersionMinor, TcgEfiSpecIdEventStruct->specErrata)
        );
  ...
  DigestSize =
    (TCG_EfiSpecIdEventAlgorithmSize *) ((UINT8 *) TcgEfiSpecIdEventStruct + sizeof (*TcgEfiSpecIdEventStruct) +
  ...
  )
{
  ...
  return sizeof (TCG_EfiSpecIDEventStruct) + sizeof (UINT32) +
         (NumberOfAlgorithms * sizeof (TCG_EfiSpecIdEventAlgorithmSize)) + sizeof (UINT8) + (*VendorInfoSize);
}
  ...
  )
{
  ...
  switch (EventLogFormat) {
    ...
    case EFI_TCG2_EVENT_LOG_FORMAT_TCG_2:
      ...
      TcgPcrEvent2 =
        (TCG_PCR_EVENT2 *) ((UINTN) TcgEfiSpecIdEventStruct + GetTcgEfiSpecIdEventStructSize (TcgEfiSpecIdEventStruct));
  ...
  )
{
  ...
  if (mTcgDxeData.GetEventLogCalled[Index]) {
    ...
    if (Status == EFI_OUT_OF_RESOURCES) {
      ...
      DEBUG (
            (EFI_D_INFO, "FinalEventsTable->NumberOfEvents - 0x%x\n",
             (mTcgDxeData.FinalEventsTable[Index])->NumberOfEvents)
            );
  ...
  )
{
  ...
  for (Index = 0; Index < Count; Index++) {
    ...
    if (IsHashAlgSupportedInHashAlgorithmMask (HashAlg, HashAlgorithmMask)) {
      ...
      DEBUG (
            (DEBUG_ERROR, "WARNING: CopyDigestListBinToBuffer Event log has HashAlg unsupported by PCR bank (0x%x)\n",
             HashAlg)
            );
    }
      ...
      switch (mTcg2EventInfo[Index].LogFormat) {
        ...
        case EFI_TCG2_EVENT_LOG_FORMAT_TCG_2:
          ...
          Status = TcgDxeLogEvent (
                                   mTcg2EventInfo[Index].LogFormat,
                                   &TcgPcrEvent2,
                                   sizeof (TcgPcrEvent2.PCRIndex) + sizeof (TcgPcrEvent2.EventType) + GetDigestListBinSize (
                                                                                                         DigestBuffer
                                                                                                         ) +
  ...
  )
{
  ...
  TCG_EfiSpecIDEventStruct  *TcgEfiSpecIdEventStruct;
  UINT8                     TempBuf[sizeof (TCG_EfiSpecIDEventStruct) + sizeof (UINT32) +
        ...
        *VendorInfoSize = 0;
        ...
        while (GuidHob.Guid != NULL) {
          ...
          Status = TcgDxeLogEvent (
                                   mTcg2EventInfo[Index].LogFormat,
                                   &NoActionEvent,
                                   sizeof (NoActionEvent.PCRIndex) + sizeof (NoActionEvent.EventType) + GetDigestListBinSize (
                                                                                                             &
                                                                                                             NoActionEvent
                                                                                                               .Digests
                                                                                                             ) +
        ...
        if (GuidHob.Guid != NULL) {
          ...
          DEBUG (
                (DEBUG_INFO, "SetupEventLog: Set Locality from HOB into StartupLocalityEvent 0x%02x\n",
  ...
  for (Index = 0; Index < sizeof (mTcg2EventInfo)/sizeof (mTcg2EventInfo[0]); Index++) {
    if ((mTcgDxeData.BsCap.SupportedEventLogs & mTcg2EventInfo[Index].LogFormat) != 0) {
      if (mTcg2EventInfo[Index].LogFormat == EFI_TCG2_EVENT_LOG_FORMAT_TCG_2) {
        ...
        mTcgDxeData.FinalEventLogAreaStruct[Index].Laml = PcdGet32 (PcdTcg2FinalLogAreaLen) -
  ...
  )
{
  ...
  if (PcdGet32 (PcdTcg2NumberOfPCRBanks) == 0) {
    ...
    if (PcdGet32 (PcdTcg2NumberOfPCRBanks) > NumberOfPCRBanks) {
      DEBUG (
            (EFI_D_ERROR, "ERROR: PcdTcg2NumberOfPCRBanks(0x%x) > NumberOfPCRBanks(0x%x)\n",
