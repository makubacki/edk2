  ...
  )
{
  ...
  Status = gBS->InstallMultipleProtocolInterfaces (
                                                   ...
                                                   &gHiiVendorDevicePath, // required for HII callback allow all disks
                                                                          // to be shown in same hii
                                                   NULL
                                                   );
  ...
  )
{
  ...
  UnicodeSPrint (
               Unicode,
               StrSize (L"WARNING: ############# action needs about ####### seconds"),
               L"WARNING: %s action needs about %d seconds",
               ActionString,
  ...
  )
{
  ...
  if (OpalDisk != NULL) {
    ...
    Ret = OpalSupportGetAvailableActions (
    ...
    if (Ret == TcgResultSuccess) {
      ...
      gHiiConfiguration.SelectedDiskAvailableActions |=
        (AvailActions.PsidRevert == 1) ? HII_ACTION_PSID_REVERT : HII_ACTION_NONE;

      //
      // Always allow unlock to handle device migration
      //
      gHiiConfiguration.SelectedDiskAvailableActions |=
        (AvailActions.Unlock == 1) ? HII_ACTION_UNLOCK : HII_ACTION_NONE;
      ...
      if (!OpalFeatureEnabled (&OpalDisk->SupportedAttributes, &OpalDisk->LockingFeature)) {
        ...
        gHiiConfiguration.SelectedDiskAvailableActions |=
          (AvailActions.AdminPass == 1) ? HII_ACTION_SET_ADMIN_PWD : HII_ACTION_NONE;
        gHiiConfiguration.SelectedDiskAvailableActions |=
  ...
  )
{
  ...
  TcgResult =
    OpalGetSupportedAttributesInfo (&Session, &Dev->OpalDisk.SupportedAttributes, &Dev->OpalDisk.OpalBaseComId);
