...
HASH_TABLE  mHash[] = {
  { L"SHA224", 28, &mHashOidValue[13], 9, NULL,                 NULL,                 NULL,
    NULL                        },
  { L"SHA256", 32, &mHashOidValue[22], 9, Sha256GetContextSize, Sha256Init,           Sha256Update,
    Sha256Final                            },
  { L"SHA384", 48, &mHashOidValue[31], 9, Sha384GetContextSize, Sha384Init,           Sha384Update,
    Sha384Final                                      },
  { L"SHA512", 64, &mHashOidValue[40], 9, Sha512GetContextSize, Sha512Init,           Sha512Update,
    Sha512Final                                      }
};
  ...
  )
{
  ...
  if ((Private->FileContext->FHandle == NULL) || (Private->FileContext->FileName == NULL) ||
      (Private->SignatureGUID == NULL)) {
    return EFI_INVALID_PARAMETER;
  }
  ...
  )
{
  ...
  if (  (NtHeader32->FileHeader.Machine == EFI_IMAGE_MACHINE_IA32)
    ...
    mSecDataDir =
      (EFI_IMAGE_SECURITY_DATA_DIRECTORY *)&(NtHeader32->OptionalHeader.DataDirectory[EFI_IMAGE_DIRECTORY_ENTRY_SECURITY
                                             ]);
    ...
    mSecDataDir =
      (EFI_IMAGE_SECURITY_DATA_DIRECTORY *)&(NtHeader64->OptionalHeader.DataDirectory[EFI_IMAGE_DIRECTORY_ENTRY_SECURITY
                                             ]);
  } else {
    return EFI_UNSUPPORTED;
  }
  ...
  )
{
  ...
  if (DelType == Delete_Signature_List_All) {
    ...
    while ((RemainingSize > 0) && (RemainingSize >= ListWalker->SignatureListSize) &&
           ListIndex < PrivateData->ListIndex) {
    ...
    if (CheckedCount < SIGNATURE_DATA_COUNTS (ListWalker) && DelType == Delete_Signature_Data) {
      ...
      DataWalker =
        (EFI_SIGNATURE_DATA *)((UINT8 *)ListWalker + sizeof (EFI_SIGNATURE_LIST) + ListWalker->SignatureHeaderSize);
