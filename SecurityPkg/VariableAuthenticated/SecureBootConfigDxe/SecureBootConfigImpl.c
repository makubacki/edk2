  ...
  )
{
  ...
  KekSigListSize = sizeof (EFI_SIGNATURE_LIST) + sizeof (EFI_SIGNATURE_DATA) -
                   1 + X509DataSize;
  KekSigList = (EFI_SIGNATURE_LIST *)AllocateZeroPool (KekSigListSize);
  if (KekSigList == NULL) {
    Status = EFI_OUT_OF_RESOURCES;
  ...
  )
{
  ...
  SigDBCertData = (EFI_SIGNATURE_DATA *)((UINT8 *)SigDBCert +
                                         sizeof (EFI_SIGNATURE_LIST));
  ...
  )
{
  ...
  //
  SignatureSize = sizeof (EFI_SIGNATURE_DATA) - 1 + sizeof (EFI_TIME) +
  ...
  )
{
  ...
  SigDBCert->SignatureListSize   = (UINT32)SigDBSize;
  SigDBCert->SignatureHeaderSize = 0;
  SigDBCert->SignatureSize       = sizeof (EFI_SIGNATURE_DATA) - 1 +
                                   (UINT32)mImageDigestSize;
      ...
      default:
                                  ...
                                  OPTION_CONFIG_RANGE)))
        {
          ...
          Private->ListIndex = QuestionId - OPTION_SIGNATURE_LIST_QUESTION_ID;
                   ...
                   (QuestionId < (OPTION_SIGNATURE_DATA_QUESTION_ID +
                                  OPTION_CONFIG_RANGE)))
        {
          if (Private->CheckArray[QuestionId -
            ...
            IfrNvData->CheckedDataCount++;
            Private->CheckArray[QuestionId -
                                OPTION_SIGNATURE_DATA_QUESTION_ID] = TRUE;
          }
