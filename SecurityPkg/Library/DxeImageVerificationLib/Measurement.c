  ...
  )
{
  ...
  if (mMeasuredAuthorityCount == mMeasuredAuthorityCountMax) {
    ...
    NewMeasuredAuthorityList =
      AllocateZeroPool (sizeof (VARIABLE_RECORD) * (mMeasuredAuthorityCountMax + MEASURED_AUTHORITY_COUNT_MAX));
  ...
  )
{
  ...
  DEBUG (
        (EFI_D_INFO, "DxeImageVerification: MeasureVariable (Pcr - %x, EventType - %x, ", (UINTN) 7,
