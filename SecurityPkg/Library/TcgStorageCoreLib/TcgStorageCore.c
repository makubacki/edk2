  ...
  )
{
  ...
  if (CreateStruct->ComPacket != NULL ||
    ...
    DEBUG (
           (DEBUG_INFO, "unexpected state: ComPacket=%p CurPacket=%p CurSubPacket=%p\n", CreateStruct->ComPacket,
  ...
  )
{
  ...
  if (CreateStruct->ComPacket == NULL ||
    ...
    DEBUG (
          (DEBUG_INFO, "unexpected state: ComPacket=%p CurPacket=%p CurSubPacket=%p\n", CreateStruct->ComPacket,
           CreateStruct->CurPacket, CreateStruct->CurSubPacket)
          );
    return (TcgResultFailureInvalidAction);
  }
  ...
  CreateStruct->CurPacket =
    (TCG_PACKET *) (CreateStruct->ComPacket->Payload + SwapBytes32 (CreateStruct->ComPacket->LengthBE));
  ...
  )
{
  ...
  CreateStruct->CurSubPacket =
    (TCG_SUB_PACKET *) (CreateStruct->CurPacket->Payload + SwapBytes32 (CreateStruct->CurPacket->LengthBE));
  ...
  )
{
  ...
  PadSize = TCG_SUBPACKET_ALIGNMENT -
  ...
  )
{
  ...
  if ((DataSize == 1) &&
      ...
      ((SignOrCont != 0 &&
        ((TCG_TOKEN_TINYATOM_SIGNED_MIN_VALUE <= *(INT8 *) Data) &&
         (*(INT8 *) Data <= TCG_TOKEN_TINYATOM_SIGNED_MAX_VALUE))) ||
  ...
  if (DataSize <= TCG_TOKEN_MEDIUMATOM_MAX_BYTE_SIZE) {
    ...
    MediumAtom.MediumAtomBits.LengthHigh =
      (DataSize >> TCG_MEDIUM_ATOM_LENGTH_HIGH_SHIFT) & TCG_MEDIUM_ATOM_LENGTH_HIGH_MASK;
    return TcgAddRawTokenData (
                             CreateStruct,
                             &MediumAtom,
  ...
  if (ParseStruct->ComPacket == NULL ||
    ...
    DEBUG (
          (DEBUG_INFO, "unexpected state: ComPacket=%p CurPacket=%p CurSubPacket=%p\n", ParseStruct->ComPacket,
           ParseStruct->CurPacket, ParseStruct->CurSubPacket)
          );
    return TcgResultFailureInvalidAction;
  }
  ...
  )
{
  ...
  switch (TcgToken->Type) {
    ...
    case TcgTokenTypeMediumAtom:
    {
      ...
      *DataLength   =
        (MediumAtom->MediumAtomBits.LengthHigh <<
         TCG_MEDIUM_ATOM_LENGTH_HIGH_SHIFT) | MediumAtom->MediumAtomBits.LengthLow;
      return TcgResultSuccess;
    }
