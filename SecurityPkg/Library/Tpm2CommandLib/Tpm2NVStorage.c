  ...
  )
{
  ...
  NvNameSize =
    SwapBytes16 (
      ReadUnaligned16 (
        (UINT16 *)((UINT8 *)&RecvBuffer + sizeof (TPM2_RESPONSE_HEADER) + sizeof (UINT16) +
                   NvPublicSize)
        )
      );
  ...
  WriteUnaligned32 (
    ...
    SwapBytes32 (ReadUnaligned32 ((UINT32 *)&NvPublic->nvPublic.attributes))
    );
  ...
  CopyMem (
    NvName->name,
    (UINT8 *)&RecvBuffer + sizeof (TPM2_RESPONSE_HEADER) + sizeof (UINT16) + NvPublicSize + sizeof (UINT16),
    NvNameSize
    );
