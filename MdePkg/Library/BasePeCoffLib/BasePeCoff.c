  ...
  )
{
  ...
  if (Hdr.Te->Signature == EFI_TE_IMAGE_HEADER_SIGNATURE) {
    ...
    ImageContext->SizeOfHeaders    = sizeof (EFI_TE_IMAGE_HEADER) + (UINTN) Hdr.Te->BaseOfCode -
    ...
    if (Hdr.Pe32->OptionalHeader.Magic == EFI_IMAGE_NT_OPTIONAL_HDR32_MAGIC) {
      ...
      //
      HeaderWithoutDataDir = sizeof (EFI_IMAGE_OPTIONAL_HEADER32) - sizeof (EFI_IMAGE_DATA_DIRECTORY) *
      ...
      if ((Hdr.Pe32->OptionalHeader.SizeOfImage - SectionHeaderOffset) / EFI_IMAGE_SIZEOF_SECTION_HEADER <=
      ...
      if ((Hdr.Pe32->OptionalHeader.SizeOfHeaders - SectionHeaderOffset) / EFI_IMAGE_SIZEOF_SECTION_HEADER <
      ...
      // OptionalHeader.NumberOfRvaAndSizes * sizeof (EFI_IMAGE_DATA_DIRECTORY) will not overflow.
      //
      HeaderWithoutDataDir = sizeof (EFI_IMAGE_OPTIONAL_HEADER64) - sizeof (EFI_IMAGE_DATA_DIRECTORY) *
                             EFI_IMAGE_NUMBER_OF_DIRECTORY_ENTRIES;
      ...
      if ((Hdr.Pe32Plus->OptionalHeader.SizeOfImage - SectionHeaderOffset) / EFI_IMAGE_SIZEOF_SECTION_HEADER <=
      ...
      if ((Hdr.Pe32Plus->OptionalHeader.SizeOfHeaders - SectionHeaderOffset) / EFI_IMAGE_SIZEOF_SECTION_HEADER <
        ...
        if (Hdr.Pe32Plus->OptionalHeader.DataDirectory[EFI_IMAGE_DIRECTORY_ENTRY_SECURITY].Size != 0) {
          ...
          Status   = ImageContext->ImageRead (
                                              ...
                                              Hdr.Pe32Plus->OptionalHeader.DataDirectory[
                                                                             EFI_IMAGE_DIRECTORY_ENTRY_SECURITY].VirtualAddress +
                                              Hdr.Pe32Plus->OptionalHeader.DataDirectory[
                                                                              EFI_IMAGE_DIRECTORY_ENTRY_SECURITY].Size - 1,
                                              &Size,
                                              &BufferData
                                              );
          if (RETURN_ERROR (Status) || (Size != ReadSize)) {
    ...
    if (NumberOfRvaAndSizes > EFI_IMAGE_DIRECTORY_ENTRY_DEBUG) {
      ...
      for (Index = 0; Index < Hdr.Pe32->FileHeader.NumberOfSections; Index++) {
        ...
        if (DebugDirectoryEntryRva >= SectionHeader.VirtualAddress &&
          ...
          DebugDirectoryEntryFileOffset = DebugDirectoryEntryRva - SectionHeader.VirtualAddress +
        ...
        for (Index = 0; Index < ResourceDirectory->NumberOfNamedEntries; Index++) {
          if (ResourceDirectoryEntry->u1.s.NameIsString) {
            ...
            ResourceDirectoryString =
              (EFI_IMAGE_RESOURCE_DIRECTORY_STRING *) (Base + ResourceDirectoryEntry->u1.s.NameOffset);
            ...
            if (ResourceDirectoryString->Length == 3 &&
              ...
              if (ResourceDirectoryEntry->u2.s.DataIsDirectory) {
                ...
                ResourceDirectory =
                  (EFI_IMAGE_RESOURCE_DIRECTORY *) (Base + ResourceDirectoryEntry->u2.s.OffsetToDirectory);
              ...
              if (!ResourceDirectoryEntry->u2.s.DataIsDirectory) {
                ...
                ImageContext->HiiResourceData = (PHYSICAL_ADDRESS) (UINTN) PeCoffLoaderImageAddress (
                                                                                                    ImageContext,
  ...
  )
{
  ...
  if (NumberOfRvaAndSizes > EFI_IMAGE_DIRECTORY_ENTRY_BASERELOC) {
    ...
    if ((RelocDir != NULL) && (RelocDir->Size > 0)) {
      RelocBase =
        (EFI_IMAGE_BASE_RELOCATION *) PeCoffLoaderImageAddress (&ImageContext, RelocDir->VirtualAddress, 0);
      RelocBaseEnd = (EFI_IMAGE_BASE_RELOCATION *) PeCoffLoaderImageAddress (
                                                                             &ImageContext,
                                                                             RelocDir->VirtualAddress + RelocDir->Size - 1,
                                                                             0
