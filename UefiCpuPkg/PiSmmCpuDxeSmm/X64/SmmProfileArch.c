  ...
  )
{
  ...
  if ((mPFPageUplink[mPFPageIndex] != NULL) &&
      ((*mPFPageUplink[mPFPageIndex] & ~mAddressEncMask & PHYSICAL_ADDRESS_MASK) == Address)) {
  ...
  )
{
  ...
  if (!Existed) {
    if (IsAddressValid (PFAddress, &Nx)) {
      ...
      // Will create a page entry for this page fault address, make page table entry as present/rw and
      // execution-disable.
