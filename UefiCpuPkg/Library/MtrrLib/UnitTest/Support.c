  ...
  while (Count-- != 0) {
    ...
    if (  (Range->BaseAddress <= Ranges[Count].BaseAddress &&
       ...
       || (Ranges[Count].BaseAddress <= Range->BaseAddress &&
           Range->BaseAddress < Ranges[Count].BaseAddress + Ranges[Count].Length)) {
      return TRUE;
    }
  }
  ...
  )
{
  ...
  //
  for (Index = UcCount + WtCount + WbCount + WpCount; Index < UcCount + WtCount + WbCount + WpCount + WcCount;
       Index++) {
  ...
  for (Index = 0, AllRangePiecesCountActual = 0; Index < AllEndPointsCount - 1; Index++) {
    ...
    switch (OverlapFlagRelation) {
      case 0:   // [1, 2]
        ...
        AllRangePieces[AllRangePiecesCountActual].Length = AllEndPointsInclusive[Index + 1] -
                                                           AllEndPointsInclusive[Index] + 1;
        AllRangePiecesCountActual++;
        break;
      case 1:   // [1, 2)
        AllRangePieces[AllRangePiecesCountActual].BaseAddress = AllEndPointsInclusive[Index];
        AllRangePieces[AllRangePiecesCountActual].Length = (AllEndPointsInclusive[Index + 1] - 1) -
                                                           AllEndPointsInclusive[Index] + 1;
        AllRangePiecesCountActual++;
        break;
      case 2:   // (1, 2]
