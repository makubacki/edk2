  ...
  )
{
  ...
  *(UINTN *) (UINTN) gStmSmiStack = CpuIndex;
  ...
  Psd->StmProtectionExceptionHandler.SpeRsp =
    (UINT64) (UINTN) AllocatePages (EFI_SIZE_TO_PAGES (PcdGet32 (PcdCpuSmmStmExceptionStackSize)));
  Psd->StmProtectionExceptionHandler.SpeRsp +=
  ...
  )
{
  ...
  for (Index = 0; Index < gSmst->NumberOfCpus; Index++) {
    Psd =
      (TXT_PROCESSOR_SMM_DESCRIPTOR *) ((UINTN) gSmst->CpuSaveState[Index] - SMRAM_SAVE_STATE_MAP_OFFSET +
                                        TXT_SMM_PSD_OFFSET);
  ...
  switch (Resource->Header.RscType) {
    ...
    case PCI_CFG_RANGE:
      ...
      if (CompareMem (
                   ...
                   sizeof (STM_PCI_DEVICE_PATH_NODE) * (Resource->PciCfg.LastNodeIndex + 1)
                   ) != 0) {
        return FALSE;
      }
  ...
  )
{
  ...
  for (Index = 0; Index < Count; Index++) {
    ...
    switch (Resource->Header.RscType) {
      ...
      case PCI_CFG_RANGE:
        DEBUG (
              (DEBUG_INFO, "ValidateResource - PCI (0x%02x, 0x%08x, 0x%02x, 0x%02x)\n",
               Resource->PciCfg.OriginatingBusNumber, Resource->PciCfg.LastNodeIndex,
               Resource->PciCfg.PciDevicePath[0].PciDevice, Resource->PciCfg.PciDevicePath[0].PciFunction)
              );
        if (Resource->Header.Length !=
            sizeof (STM_RSC_PCI_CFG_DESC) + (sizeof (STM_PCI_DEVICE_PATH_NODE) * Resource->PciCfg.LastNodeIndex)) {
          return FALSE;
        }
  ...
  )
{
  ...
  if (MinMsegSize > mMsegSize) {
    ...
    DEBUG (
          (DEBUG_ERROR, "  StmHeader->SwStmHdr.StaticImageSize             = %08x\n",
           StmHeader->SwStmHdr.StaticImageSize)
          );
    DEBUG (
          (DEBUG_ERROR, "  StmHeader->SwStmHdr.AdditionalDynamicMemorySize = %08x\n",
           StmHeader->SwStmHdr.AdditionalDynamicMemorySize)
          );
    DEBUG (
          (DEBUG_ERROR, "  StmHeader->SwStmHdr.PerProcDynamicMemorySize    = %08x\n",
