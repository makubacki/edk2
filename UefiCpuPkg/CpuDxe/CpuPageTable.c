  ...
  )
{
  ...
  if (PageAttribute == Page2M) {
    ...
    if ((SplitAttribute == Page2M || SplitAttribute == Page4K)) {
      ...
      for (Index = 0; Index < SIZE_4KB / sizeof (UINT64); Index++) {
        NewPageEntry[Index] = (BaseAddress + SIZE_2MB * Index) | AddressEncMask | IA32_PG_PS |
  ...
  )
{
  ...
  switch(CurrentPagingContext.MachineType) {
    case IMAGE_FILE_MACHINE_I386:
      ...
      if ((CurrentPagingContext.ContextData.Ia32.Attributes & PAGE_TABLE_LIB_PAGING_CONTEXT_IA32_X64_ATTRIBUTES_PAE) ==
          0) {
